...
def _metadata_ok(d: str, output_hash: str | None, counts: dict) -> bool:
    try:
        ...
        snapshots_valid = (
            isinstance(source_snapshots, dict)
            and isinstance(snapshot_methods, dict)
            and -300 <= snapshot_age <= HISTORICAL_SNAPSHOT_MAX_AGE_DAYS * 86400
            and all(isinstance(canon, str) and isinstance(sha, str)
                    and re.fullmatch(r"[0-9a-f]{40}", sha)
                    and snapshot_methods.get(canon) in
                    ("resolved-api", "resolved-ref-service", "already-sha")
                    for canon, sha in source_snapshots.items())
        )
        return (
...
def _load_source_snapshots(d: str, output_hash: str | None) -> dict[str, str]:
    ...
    try:
        ...
        if age < -300 or age > HISTORICAL_SNAPSHOT_MAX_AGE_DAYS * 86400:
            return {}
        if not all(isinstance(canon, str) and isinstance(sha, str)
                   and re.fullmatch(r"[0-9a-f]{40}", sha)
                   and methods.get(canon) in
                   ("resolved-api", "resolved-ref-service", "already-sha")
