...
DNS_MAX_QUERY_LINE = 4096


def _dns_resolver_worker_main():
    while True:
        line = sys.stdin.buffer.readline(DNS_MAX_QUERY_LINE + 1)
        if not line:
            return
        if len(line) > DNS_MAX_QUERY_LINE:
            print(json.dumps({"error": "RequestTooLong"}, separators=(",", ":")), flush=True)
            return
        try:
            host = json.loads(line.decode("utf-8"))
            ...
            print(json.dumps({"addresses": addresses}, separators=(",", ":")), flush=True)
        except Exception as exc:
            print(json.dumps({"error": type(exc).__name__}, separators=(",", ":")), flush=True)


if __name__ == "__main__" and sys.argv[1:] == [_DNS_RESOLVER_WORKER_ARG]:
    _dns_resolver_worker_main()
    raise SystemExit(0)
...
try:
    ...
    sys.exit("缺少依赖 requests：请先 `pip install requests==2.34.2 urllib3==2.7.0`")
...
if (_requests_version is None or _requests_version[0] != 2 or _requests_version < (2, 34, 2)
        or _urllib3_version is None or _urllib3_version[0] != 2 or _urllib3_version < (2, 7, 0)):
    sys.exit(f"依赖版本不兼容：requests={requests.__version__}, urllib3={urllib3.__version__}；"
             "需要 requests>=2.34.2,<3 和 urllib3>=2.7.0,<3")
...
TIMEOUT = (5, 20)
DNS_TIMEOUT = 5
DNS_MAX_OUTSTANDING = 4
MAX_WORKERS = 12
MAX_RESP_SIZE = 2 * 1024 * 1024
MAX_TOTAL_INPUT_SIZE = 64 * 1024 * 1024
MAX_ITEMS_PER_SOURCE = 2000
MIN_SOURCE_ITEM_VALID_RATIO = 0.90
MAX_TOTAL_CANDIDATES = 50000
MAX_OUTPUT_FILE_SIZE = 25 * 1024 * 1024
MAX_TOTAL_OUTPUT_SIZE = 64 * 1024 * 1024
MAX_SOURCE_LINKS = 5000
MAX_UNIQUE_SOURCES = 180
MAX_LINK_FILE_SIZE = 1024 * 1024
GITHUB_API_WORKERS = 8
GITHUB_REF_WORKERS = 8
GITHUB_REF_TIMEOUT = 20
MAX_GITHUB_API_RESPONSE = 1024 * 1024
MAX_SNAPSHOT_RESOLUTION_WINDOW = 60
HISTORICAL_SNAPSHOT_MAX_AGE_DAYS = 7
MAX_REDIRECTS = 5
...
MAX_CUMULATIVE_DELETE_RATIO = 0.10
NEW_ITEM_CONCENTRATION_MIN = 20
MAX_NEW_ITEM_SOURCE_SHARE = 0.60
MAX_STALE_RATIO = 0.50
GROUP_DEADLINE = 90
SNAPSHOT_METHOD_TRUST = {
    "resolved-api": "strict",
    "resolved-ref-service": "strict",
    "already-sha": "strict",
    "resolved-history": "degraded",
}
STRICT_SNAPSHOT_METHODS = frozenset(
    method for method, trust in SNAPSHOT_METHOD_TRUST.items() if trust == "strict")
IMMUTABLE_SNAPSHOT_METHODS = frozenset(SNAPSHOT_METHOD_TRUST)
...
KNOWN_PLAIN_STRING_FIELDS = ("rawBaseUrl", "subjectFormatId", "channelFormatId",
                             ...
                             "referer", "userAgent")
ALLOWED_ITEM_FIELDS = frozenset(("factoryId", "version", "arguments"))
ALLOWED_ARGUMENT_FIELDS = frozenset(("name", "description", "iconUrl", "searchConfig", "tier",
                                     "channelTiers", "rssUrl", "searchUrl"))
ALLOWED_SEARCH_CONFIG_FIELDS = frozenset(
    ("searchUrl", "rssUrl") + KNOWN_REGEX_FIELDS + SELECTOR_FIELDS + KNOWN_BOOL_FIELDS
    + KNOWN_INT_FIELDS + KNOWN_STRING_LIST_FIELDS + KNOWN_OBJECT_FIELDS
    + KNOWN_PLAIN_STRING_FIELDS
)
...
MAX_LEN_URL = 8192
PRIVATE_HOST_SUFFIXES = ("localhost", "local", "internal", "lan", "home", "localdomain")
...
def is_private_hostname(host: str) -> bool:
    ...
    return any(value == suffix or value.endswith("." + suffix)
...
def _resolver_command() -> list[str]:
    ...
    return [sys.executable, "-I", "-u", script, _DNS_RESOLVER_WORKER_ARG]


def _resolver_environment() -> dict[str, str]:
    resolver_env = {"PYTHONIOENCODING": "utf-8", "PYTHONUTF8": "1"}
...
class _DNSResolverWorker:
    ...
    @staticmethod
    def _read_responses(proc, responses):
        try:
    ...
    def start(self):
        self.close()
        resolver_env = _resolver_environment()
        # 命令固定为当前解释器、当前脚本和内部入口参数，不接收外部argv，也不经过shell。
        proc = subprocess.Popen(  # nosec B603
            _resolver_command(),
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
            text=True, bufsize=1, env=resolver_env,
        )
        responses = queue.Queue()
        reader = threading.Thread(target=self._read_responses, args=(proc, responses), daemon=True)
        self.proc = proc
    ...
    def close(self):
        ...
        if proc is None:
            if reader and reader is not threading.current_thread():
                ...
                if reader.is_alive():
                    raise RuntimeError("resolver-reader-did-not-exit")
            return
        try:
            if proc.stdin:
                proc.stdin.close()
    ...
    def query(self, host: str, timeout: float):
        if self.proc is None or self.proc.poll() is not None:
            self.start()
        try:
            self.proc.stdin.write(json.dumps(host) + "\n")
            self.proc.stdin.flush()
        except Exception as exc:
            raise RuntimeError("resolver-write-failed") from exc
        try:
            line = self.responses.get(timeout=timeout)
        except queue.Empty as exc:
            raise TimeoutError("resolver-timeout") from exc
        if line is None:
            raise RuntimeError("resolver-exited")
        payload = json.loads(line)
        if not isinstance(payload, dict):
            raise RuntimeError("resolver-invalid-response")
        if payload.get("error"):
            return None, "dns-fail"
        return payload.get("addresses"), None
...
def _get_dns_pool():
    ...
    with _dns_pool_lock:
        if _dns_pool is None:
            workers = []
            try:
                for _ in range(DNS_MAX_OUTSTANDING):
                    workers.append(_DNSResolverWorker())
            except Exception:
                for worker in workers:
                    worker.close()
                raise
            pool = queue.LifoQueue(maxsize=DNS_MAX_OUTSTANDING)
            for worker in workers:
                pool.put(worker)
            _dns_pool = pool
        return _dns_pool
...
atexit.register(_close_dns_pool)
...
def _bounded_getaddrinfo(host: str, deadline: float | None = None):
    ...
    restart = False
    try:
        addresses, error = worker.query(host, wait)
        if error:
            return None, error
        ...
        for address in addresses:
            ...
            try:
                ...
                restart = True
                return None, "dns-fail"
            infos.append((None, None, None, None, (address, 0)))
        return infos, None
    except TimeoutError:
        restart = True
        return None, "dns-timeout"
...
def _choose_public_ip(host: str, infos):
    ...
    if not public_ips:
        return None
    signature = tuple(public_ips)
    with _dns_rotation_lock:
        previous = _dns_rotation.get(host)
        index = previous[1] if previous and previous[0] == signature else 0
        _dns_rotation[host] = (signature, index + 1)
...
_GITHUB_NAME_RE = re.compile(r"[A-Za-z0-9_.-]{1,100}\Z")


def _decode_github_segment(raw: str) -> str | None:
    value = raw
    for _ in range(5):
...
def _github_path_segments(path: str) -> list[str] | None:
    if not path.startswith("/") or "\\" in path:
        return None
    raw_parts = path[1:].split("/")
    if not raw_parts or any(part == "" for part in raw_parts):
        return None
    parts = [_decode_github_segment(part) for part in raw_parts]
...
def _github_ref_safe(ref: str) -> bool:
    ...
    if ".." in ref or "@{" in ref or "//" in ref:
        return False
    return not any(ord(ch) < 32 or ord(ch) == 127 or ch in " ~^:?*[\\" for ch in ref)


def _github_endpoint_ok(parsed, hostname: str) -> bool:
    try:
        port = parsed.port
    except ValueError:
        return False
    return (
        ...
        and (parsed.hostname or "").lower().rstrip(".") == hostname
...
class GitHubRef:
    ...
    def __init__(self, owner, repo, ref, path):
        if not (_GITHUB_NAME_RE.fullmatch(owner or "") and _GITHUB_NAME_RE.fullmatch(repo or "")):
            raise ValueError("GitHub owner/repo 非法")
        if not _github_ref_safe(ref):
            raise ValueError("GitHub ref 非法")
        path_parts = path.split("/") if isinstance(path, str) else []
        if not path_parts or any(_decode_github_segment(part) != part for part in path_parts):
            raise ValueError("GitHub path 非法")
        self.owner, self.repo, self.ref, self.path = owner, repo, ref, path

    def raw_url(self):
        owner = quote(self.owner, safe="")
        repo = quote(self.repo, safe="")
        ref = "/".join(quote(part, safe="") for part in self.ref.split("/"))
        path = "/".join(quote(part, safe="") for part in self.path.split("/"))
        return f"https://raw.githubusercontent.com/{owner}/{repo}/{ref}/{path}"

    def same_resource(self, other, include_ref: bool = True) -> bool:
        return (
            isinstance(other, GitHubRef)
            and self.owner.casefold() == other.owner.casefold()
            and self.repo.casefold() == other.repo.casefold()
            and self.path == other.path
            and (not include_ref or self.ref.casefold() == other.ref.casefold())
        )
...
def _build_github_ref(owner: str, repo: str, ref: str, path_parts: list[str]):
    try:
        return GitHubRef(owner, repo, fold_ref(ref), "/".join(path_parts))
    except (TypeError, ValueError):
        return None


def _split_explicit_ref(parts: list[str], index: int):
    if parts[index] == "refs":
        if len(parts) <= index + 3 or parts[index + 1] not in ("heads", "tags"):
            return None
...
def parse_github_raw(url: str):
    try:
        ...
        return None
    if not _github_endpoint_ok(parsed, "raw.githubusercontent.com"):
        return None
    parts = _github_path_segments(parsed.path)
    if parts is None or len(parts) < 4:
        return None
    split = _split_explicit_ref(parts, 2)
    return _build_github_ref(parts[0], parts[1], *split) if split else None
...
def parse_github_com(url: str):
    try:
        ...
        return None
    if not _github_endpoint_ok(parsed, "github.com"):
        return None
    parts = _github_path_segments(parsed.path)
    if parts is None or len(parts) < 5 or parts[2] not in ("raw", "blob"):
        return None
    split = _split_explicit_ref(parts, 3)
    return _build_github_ref(parts[0], parts[1], *split) if split else None
...
def parse_jsdelivr(url: str):
    try:
        ...
        return None
    if not _github_endpoint_ok(parsed, "cdn.jsdelivr.net"):
        return None
    parts = _github_path_segments(parsed.path)
    if parts is None or len(parts) < 4 or parts[0] != "gh" or "@" not in parts[2]:
        return None
    repo, first_ref = parts[2].split("@", 1)
    if not repo or not first_ref:
        return None
    ref_parts = [first_ref, *parts[3:]]
    split = _split_explicit_ref(ref_parts, 0)
    return _build_github_ref(parts[1], repo, *split) if split else None
...
def normalize(url: str) -> str:
    ...
    if is_private_hostname(up.hostname):
        raise ValueError(f"上游配置链接不得使用内网主机名: {u[:60]}")
...
def has_unsupported_scheme(url: str) -> bool:
...
def load_strict_json_bytes(data: bytes):
    return json.loads(data.decode("utf-8-sig"), parse_constant=_reject_json_constant,
                      parse_float=_strict_json_float, object_pairs_hook=_strict_json_object)


def _write_bytes_atomic(path: str, data: bytes):
    parent = os.path.dirname(path) or "."
    if os.path.lexists(parent) and os.path.islink(parent):
        raise RuntimeError(f"原子写入目录不得是符号链接: {parent}")
    os.makedirs(parent, exist_ok=True)
    fd, tmp = tempfile.mkstemp(prefix=f".{os.path.basename(path)}.tmp-", dir=parent)
    try:
...
def _write_json_atomic(path: str, obj):
    data = json.dumps(
        obj, ensure_ascii=False, indent=1, allow_nan=False).encode("utf-8")
    _write_bytes_atomic(path, data)


def _write_report_or_stderr(path: str, report: dict) -> bool:
    try:
        _write_json_atomic(path, report)
        return True
    except Exception as exc:
        fallback = {"report_write_error": f"{type(exc).__name__}: {exc}", "report": report}
        try:
            print(json.dumps(fallback, ensure_ascii=False, allow_nan=False), file=sys.stderr)
        except Exception:
            print("report_write_error", file=sys.stderr)
        return False
...
def load_json_bytes(data: bytes):
    try:
        ...
        try:
            ...
            raise ValueError("JSON 既不是有效 UTF-8，也不是有效 GBK") from exc
    try:
        obj = json.loads(text, parse_constant=_reject_json_constant,
                         parse_float=_strict_json_float,
                         object_pairs_hook=_strict_json_object)
    except ValueError as exc:
        raise ValueError(f"{encoding} JSON 语法、数字或对象键非法: {exc}") from exc
    if not isinstance(extract(obj), list):
        raise ValueError(f"{encoding} JSON 结构无效：缺少 mediaSources 列表")
    return obj
...
def validate_item(m) -> tuple[bool, list[str]]:
    ...
    unknown_item_fields = sorted(set(m) - ALLOWED_ITEM_FIELDS)
    if unknown_item_fields:
        problems.append(f"未知条目字段: {unknown_item_fields}")
    ...
    def reject_control_chars(obj, path):
        if isinstance(obj, str):
            if _has_control_chars(obj):
                problems.append(f"字符串含控制字符: {path}")
        elif isinstance(obj, dict):
            for key, value in obj.items():
                if isinstance(key, str) and _has_control_chars(key):
                    problems.append(f"字段名含控制字符: {path}")
                reject_control_chars(value, f"{path}.{key}")
        elif isinstance(obj, list):
            for index, value in enumerate(obj):
                reject_control_chars(value, f"{path}[{index}]")

    reject_control_chars(a, "arguments")
    unknown_argument_fields = sorted(set(a) - ALLOWED_ARGUMENT_FIELDS)
    if unknown_argument_fields:
        problems.append(f"未知 arguments 字段: {unknown_argument_fields}")
    name = a.get("name")
    ...
    def check_lengths(obj):
        if isinstance(obj, dict):
            for k, v in obj.items():
                if k not in ALLOWED_SEARCH_CONFIG_FIELDS:
                    problems.append(f"未知 searchConfig 字段 {name}: {k}")
                    check_lengths(v)
                elif k in KNOWN_REGEX_FIELDS:
...
def cache_path(canon: str) -> str:
    if os.path.lexists(CACHE_DIR):
        if os.path.islink(CACHE_DIR) or not os.path.isdir(CACHE_DIR):
            raise RuntimeError("cache 目录必须是真实目录且不得是符号链接")
    else:
        os.makedirs(CACHE_DIR, mode=0o700)
    return os.path.join(CACHE_DIR, hashlib.sha256(canon.encode()).hexdigest() + ".json")


def load_cache(canon: str, accepted_output_hash: str | None = None,
    ...
    try:
        ...
        if not isinstance(c, dict) or c.get("canon") != canon:
            return None
        bound_output = c.get("accepted_output_sha256")
        if bound_output is not None and (not isinstance(bound_output, str)
                                         or not re.fullmatch(r"[0-9a-f]{64}", bound_output)):
            return None
        if accepted_output_hash is not None and bound_output != accepted_output_hash:
            return None
        snapshot_sha = c.get("snapshot_sha")
        if snapshot_sha is not None and (not isinstance(snapshot_sha, str)
                                         or not re.fullmatch(r"[0-9a-f]{40}", snapshot_sha)):
            return None
        if expected_snapshot_sha is not None and snapshot_sha != expected_snapshot_sha:
            return None
        ts = c.get("ts")
...
def save_cache(canon: str, data: bytes, meta: dict, url: str,
    ...
    if accepted_output_hash is not None:
        payload["accepted_output_sha256"] = accepted_output_hash
    if snapshot_sha is not None:
        payload["snapshot_sha"] = snapshot_sha
    try:
        p = cache_path(canon)
        if os.path.islink(p):
            return False
...
def _rewrite_cache_metadata(canon: str, accepted_output_hash: str | None, refresh_ts: bool,
                            snapshot_sha: str | None = None):
    try:
        p = cache_path(canon)
        if os.path.islink(p):
        ...
        if refresh_ts:
            c["ts"] = time.time()
        if accepted_output_hash is not None:
            c["accepted_output_sha256"] = accepted_output_hash
        if snapshot_sha is not None:
            c["snapshot_sha"] = snapshot_sha
        _write_json_atomic(p, c)
        return True
    except Exception:
        return False


def touch_cache(canon: str, accepted_output_hash: str | None = None,
                snapshot_sha: str | None = None):
    return _rewrite_cache_metadata(canon, accepted_output_hash, True, snapshot_sha)


def bind_cache(canon: str, accepted_output_hash: str, snapshot_sha: str | None = None):
    return _rewrite_cache_metadata(canon, accepted_output_hash, False, snapshot_sha)


def _snapshot_cache_state(canons) -> dict[str, bytes | None]:
    state = {}
    for canon in dict.fromkeys(canons):
        path = cache_path(canon)
        if os.path.islink(path):
            raise RuntimeError(f"cache 路径不得是符号链接: {path}")
        if not os.path.exists(path):
            state[path] = None
            continue
        if not os.path.isfile(path) or os.path.getsize(path) > MAX_RESP_SIZE * 3:
            raise RuntimeError(f"cache 文件无法安全备份: {path}")
        with open(path, "rb") as handle:
            state[path] = handle.read()
    return state


def _restore_cache_state(state: dict[str, bytes | None]) -> bool:
    restored = True
...
class _InputBudget:
    ...
    def reserve_cache(self, size: int) -> bool:
        if size < 0:
            return False
        with self._lock:
            if self.cache_bytes + self.network_bytes + size > self.limit:
                return False
            self.cache_bytes += size
            return True

    def claim_network(self, maximum: int) -> int:
        with self._lock:
            remaining = self.limit - self.cache_bytes - self.network_bytes
            claimed = min(maximum, max(0, remaining))
            self.network_bytes += claimed
            return claimed

    def release_network(self, size: int):
        if size <= 0:
            return
        with self._lock:
            self.network_bytes = max(0, self.network_bytes - size)

    def totals(self) -> tuple[int, int]:
        with self._lock:
            return self.cache_bytes, self.network_bytes


def _iter_response_chunks(response, deadline: float, input_budget: _InputBudget | None):
    if input_budget is None:
        for chunk in response.iter_content(64 * 1024):
            if time.monotonic() > deadline:
                raise TimeoutError("deadline-during-body")
            if chunk:
                yield chunk
        return
    while True:
        if time.monotonic() > deadline:
            raise TimeoutError("deadline-during-body")
        allowance = input_budget.claim_network(64 * 1024)
        if allowance <= 0:
            raise _InputBudgetExceeded("total-input-too-large")
        try:
            chunk = response.raw.read(allowance, decode_content=True)
        except Exception:
            input_budget.release_network(allowance)
            raise
        if not chunk:
            input_budget.release_network(allowance)
            return
        if len(chunk) > allowance:
            raise _InputBudgetExceeded("decoder-exceeded-reserved-input")
        input_budget.release_network(allowance - len(chunk))
        yield chunk


def fetch_url(url: str, cache_meta: dict | None, deadline: float,
              expected_snapshot: GitHubRef | None = None,
              input_budget: _InputBudget | None = None):
    ...
    try:
        while redirect_count <= MAX_REDIRECTS:
            ...
            if expected_snapshot is not None and not _url_matches_snapshot(current, expected_snapshot):
                meta["error"] = "snapshot-url-mismatch"
            ...
            if is_literal_private_host(hop_host):
                ...
                meta["redirects"].append(current)
                return None, meta
            data, hop_meta, location = single_hop(
                current, cache_meta, deadline, input_budget=input_budget)
            ...
            if location is not None:
                ...
                if urlsplit(current).scheme == "https" and urlsplit(next_url).scheme != "https":
                    ...
                    return None, meta
                if expected_snapshot is not None and not _url_matches_snapshot(next_url, expected_snapshot):
                    meta["error"] = "snapshot-redirect-mismatch"
                    meta["redirects"].append(next_url)
                    return None, meta
                current = next_url
                continue
        ...
        while retry_attempt < MAX_RETRIES_PER_URL:
            ...
            try:
                ...
                with sess.get(current, timeout=request_timeout, headers=headers,
                    ...
                    size = 0
                    try:
                        for chunk in _iter_response_chunks(r, deadline, input_budget):
                            size += len(chunk)
...
def parse_retry_after(value: str | None, default: float) -> float:
    ...
    v = value.strip()
    if v.isdigit():
        if len(v) > MAX_RETRY_AFTER_DIGITS:
            return default
        try:
            return min(int(v), 15)
        except (ValueError, OverflowError):
...
def fetch_group_worker(canon: str, urls: list[str], cache_meta,
    ...
    for url in urls:
        data, meta = fetch_url(
            url, cache_meta, deadline, expected_snapshot, input_budget=input_budget)
        if data is not None:
            _, valid = try_parse(data, canon)
            if valid:
                if attempts:
                    meta["previous_attempts"] = attempts
                return data, meta
            meta["error"] = "fresh-content-invalid"
        attempts.append({"url": url, "status": meta.get("status"), "error": meta.get("error")})
        last_meta = meta
        error = meta.get("error") or ""
        if "deadline" in error or error in {"total-input-too-large", "decoder-exceeded-reserved-input"}:
            break
    if last_meta is None:
        last_meta = {"error": "no-url-attempted"}
...
def _git_ref_safe(ref: str) -> bool:
    return _github_ref_safe(ref)


def _parse_git_info_refs(data: bytes) -> dict[str, str]:
    refs = {}
    offset = 0
    while offset < len(data):
        if offset + 4 > len(data):
            raise ValueError("git pkt-line header truncated")
        header = data[offset:offset + 4]
        offset += 4
        try:
            length = int(header, 16)
        except ValueError as exc:
            raise ValueError("git pkt-line length invalid") from exc
        if length in (0, 1, 2):
            continue
        if length < 4 or offset + length - 4 > len(data):
            raise ValueError("git pkt-line body truncated")
        payload = data[offset:offset + length - 4]
        offset += length - 4
        payload = payload.rstrip(b"\n")
        if payload.startswith(b"# service="):
            continue
        payload = payload.split(b"\x00", 1)[0]
        fields = payload.split(b" ", 1)
        if len(fields) != 2:
            continue
        sha = fields[0].decode("ascii", "strict").lower()
        ref_name = fields[1].decode("utf-8", "strict")
        if re.fullmatch(r"[0-9a-f]{40}", sha):
            refs[ref_name] = sha
    return refs


def _resolve_ref_via_http(owner: str, repo: str, ref: str) -> tuple[str | None, str]:
    decoded_ref = unquote(ref)
    if not _git_ref_safe(decoded_ref):
        return None, "ref-service-invalid-ref"
    owner_q = quote(unquote(owner), safe="")
    repo_q = quote(unquote(repo), safe="")
    url = f"https://github.com/{owner_q}/{repo_q}.git/info/refs?service=git-upload-pack"
    data, meta = fetch_url(url, None, time.monotonic() + GITHUB_REF_TIMEOUT)
    if data is None:
        return None, f"ref-service-{meta.get('error') or 'failed'}"
    expected_path = f"/{unquote(owner)}/{unquote(repo)}.git/info/refs".lower()
    visited_urls = meta.get("redirects") or []
    if not visited_urls:
        return None, "ref-service-trust-boundary"
    for visited in visited_urls:
        parsed_url = urlsplit(visited)
        if ((parsed_url.hostname or "").lower().rstrip(".") != "github.com"
                or unquote(parsed_url.path).lower() != expected_path
                or parsed_url.query != "service=git-upload-pack"):
            return None, "ref-service-trust-boundary"
    try:
        found = _parse_git_info_refs(data)
    except Exception:
        return None, "ref-service-invalid-response"
    if decoded_ref.startswith("refs/heads/"):
        branch_ref = decoded_ref
        tag_ref = None
    elif decoded_ref.startswith("refs/tags/"):
        branch_ref = None
        tag_ref = decoded_ref
    else:
        branch_ref = f"refs/heads/{decoded_ref}"
        tag_ref = f"refs/tags/{decoded_ref}"
    if branch_ref and branch_ref in found:
        return found[branch_ref], "resolved-ref-service"
    if tag_ref and tag_ref + "^{}" in found:
        return found[tag_ref + "^{}"], "resolved-ref-service"
    if tag_ref and tag_ref in found:
        return found[tag_ref], "resolved-ref-service"
    return None, "ref-service-ref-not-found"
...
def resolve_commit_shas(canons: list[str]) -> tuple[dict[str, str], dict[str, str]]:
    ...
    def resolve_one(info):
        ...
        try:
            ...
            if r.status_code == 200:
                body = r.content
                if not isinstance(body, bytes) or len(body) > MAX_GITHUB_API_RESPONSE:
                    return ref, "api-invalid-response"
                try:
                    payload = load_strict_json_bytes(body)
                except Exception:
                    return ref, "api-invalid-json"
                if not isinstance(payload, dict):
                    return ref, "api-invalid-response"
                candidate_sha = payload.get("sha")
                if isinstance(candidate_sha, str) and re.fullmatch(r"[0-9a-fA-F]{40}", candidate_sha):
                    return candidate_sha.lower(), "resolved-api"
                return ref, "api-invalid-response"
    ...
    allowed = sorted((key, info) for key, info in ref_info.items() if key not in repo_refs)
    if allowed:
    ...
    needs_ref_service = [(key, ref_info[key]) for key, (_, st) in repo_refs.items()
                         if st not in ("resolved-api", "already-sha")]
    if needs_ref_service:
        with ThreadPoolExecutor(max_workers=min(GITHUB_REF_WORKERS, len(needs_ref_service))) as ex:
            futures = {ex.submit(_resolve_ref_via_http, *info): key
                       for key, info in needs_ref_service}
            for future in as_completed(futures):
                key = futures[future]
                previous_ref, previous_status = repo_refs[key]
                try:
                    resolved_sha, resolved_status = future.result()
                except Exception:
                    resolved_sha, resolved_status = None, "ref-service-error"
                if resolved_sha is not None:
                    repo_refs[key] = (resolved_sha, resolved_status)
                else:
                    repo_refs[key] = (previous_ref,
                                      f"snapshot-unresolved:{previous_status}:{resolved_status}")
...
def _url_matches_snapshot(url: str, expected: GitHubRef) -> bool:
    if not isinstance(expected, GitHubRef) or not re.fullmatch(r"[0-9a-f]{40}", expected.ref):
        return False
    try:
        parsed = parse_github_raw(url) or parse_jsdelivr(url)
        return bool(parsed and expected.same_resource(parsed, include_ref=True))
    except Exception:
        return False


def _github_snapshot_urls(canon: str, pinned_url: str) -> list[str]:
    original = parse_github_raw(canon)
    pinned = parse_github_raw(pinned_url)
    if (not original or not pinned or not re.fullmatch(r"[0-9a-f]{40}", pinned.ref)
            or not original.same_resource(pinned, include_ref=False)):
        raise ValueError(f"GitHub snapshot 无效或资源身份不匹配: {canon}")
    owner_q = quote(pinned.owner, safe="")
    repo_q = quote(pinned.repo, safe="")
    path_q = "/".join(quote(part, safe="") for part in pinned.path.split("/"))
    jsdelivr = f"https://cdn.jsdelivr.net/gh/{owner_q}/{repo_q}@{pinned.ref}/{path_q}"
    return [pinned.raw_url(), jsdelivr]
...
def _safe_dist_hash() -> str | None:
    try:
        return _dir_hash("dist") if _output_dir_structurally_valid("dist") else None
    except Exception:
        return None
...
def _output_size_problems(d: str) -> list[str]:
    ...
    if total > MAX_TOTAL_OUTPUT_SIZE:
        problems.append(f"产物总大小过大：{total} > {MAX_TOTAL_OUTPUT_SIZE}")
    return problems
...
def _snapshot_integrity_from_methods(snapshots, methods) -> str:
    if not isinstance(snapshots, dict) or not isinstance(methods, dict):
        return "invalid"
    trust_values = []
    for canon in snapshots:
        trust = SNAPSHOT_METHOD_TRUST.get(methods.get(canon))
        if trust is None:
            return "invalid"
        trust_values.append(trust)
    return "degraded" if "degraded" in trust_values else "strict"


def _snapshot_methods_valid(snapshots, methods, integrity_level: str) -> bool:
    if _snapshot_integrity_from_methods(snapshots, methods) != integrity_level:
        return False
    return all(isinstance(canon, str) and isinstance(sha, str)
               and re.fullmatch(r"[0-9a-f]{40}", sha)
               and methods.get(canon) in IMMUTABLE_SNAPSHOT_METHODS
               for canon, sha in snapshots.items())


def _metadata_ok(d: str, output_hash: str | None, counts: dict,
                 expected_snapshots: dict[str, str] | None = None) -> bool:
    try:
        latest = load_json_file(os.path.join(d, "latest.json"))
        source_base = latest.get("source_base_commit", "")
        guard_anchor = latest.get("guard_anchor")
        source_snapshots = latest.get("source_snapshots")
        snapshot_methods = latest.get("source_snapshot_methods")
        snapshots_created = latest.get("source_snapshots_created")
        snapshot_age = (time.time() - snapshots_created
                        if isinstance(snapshots_created, int) and not isinstance(snapshots_created, bool)
                        else float("inf"))
        snapshots_valid = (
            -300 <= snapshot_age <= HISTORICAL_SNAPSHOT_MAX_AGE_DAYS * 86400
            and _snapshot_methods_valid(source_snapshots, snapshot_methods,
                                        latest.get("integrity_level"))
        )
        return (
            ...
            and _guard_anchor_valid(guard_anchor)
            and snapshots_valid
            and (expected_snapshots is None or source_snapshots == expected_snapshots)
            and latest.get("integrity_level") == "strict"
        )
    except Exception:
        return False
...
def _rollback_dist_swap(backup: str | None, old_existed: bool):
    try:
        if os.path.lexists("dist"):
            _remove_path("dist")
        if old_existed and backup and os.path.lexists(backup):
...
def _output_identities(fn: str, items: list) -> set:
...
def _content_guard_problems(old_outputs: dict, new_outputs: dict) -> list[str]:
    ...
    for fn in OUTPUT_FILES:
        ...
        if not isinstance(old_ms, list) or not isinstance(new_ms, list):
            continue
        old_keys = _output_identities(fn, old_ms)
        new_keys = _output_identities(fn, new_ms)
        if not old_keys:
            continue
    ...
    if isinstance(old_all, list) and isinstance(new_all, list) and old_all:
        ...
        if replaced / len(old_fingerprints) > MAX_DELETE_RATIO:
            problems.append(f"[P0-2] all.json 内容替换 {replaced}/{len(old_fingerprints)} 超 "
                            f"{MAX_DELETE_RATIO:.0%}，拒绝覆盖")
    return problems


def _concentration_guard_problems(old_outputs: dict, new_outputs: dict,
                                  origin_by_key: dict) -> list[str]:
    old_all = old_outputs.get("all.json")
    new_all = new_outputs.get("all.json")
    if not isinstance(old_all, list) or not isinstance(new_all, list):
        return []
    old_keys = _output_identities("all.json", old_all)
    new_keys = _output_identities("all.json", new_all)
    added = new_keys - old_keys
    if len(added) < NEW_ITEM_CONCENTRATION_MIN:
        return []
    counts = {}
    for key in added:
        origin = origin_by_key.get(key, "unknown")
        counts[origin] = counts.get(origin, 0) + 1
    origin, count = max(counts.items(), key=lambda item: item[1])
    if count / len(added) > MAX_NEW_ITEM_SOURCE_SHARE:
        return [f"[P0-2] 新增条目过度集中：{origin} 贡献 {count}/{len(added)} "
                f"超 {MAX_NEW_ITEM_SOURCE_SHARE:.0%}，拒绝覆盖"]
    return []


def _make_guard_anchor(outputs: dict, created: int | None = None) -> dict:
    all_items = outputs.get("all.json") if isinstance(outputs, dict) else None
    fingerprints = sorted(sha256_json(item) for item in all_items) if isinstance(all_items, list) else []
    return {"created": int(time.time()) if created is None else int(created),
            "fingerprints": fingerprints}


def _guard_anchor_valid(anchor) -> bool:
    if not isinstance(anchor, dict):
        return False
    created = anchor.get("created")
    fingerprints = anchor.get("fingerprints")
    if isinstance(created, bool) or not isinstance(created, int):
        return False
    if created > time.time() + 300:
        return False
    if not isinstance(fingerprints, list) or not fingerprints or len(fingerprints) > MAX_TOTAL_CANDIDATES:
        return False
...
def _load_guard_anchor(d: str):
    try:
        latest = load_json_file(os.path.join(d, "latest.json"))
        anchor = latest.get("guard_anchor")
        return anchor if _guard_anchor_valid(anchor) else None
    except Exception:
        return None


def _load_source_snapshots(d: str, output_hash: str | None) -> dict[str, str]:
    if output_hash is None:
        return {}
    try:
        latest = load_json_file(os.path.join(d, "latest.json"))
        snapshots = latest.get("source_snapshots")
        methods = latest.get("source_snapshot_methods")
        created = latest.get("source_snapshots_created")
        if (latest.get("output_sha256") != output_hash or not isinstance(snapshots, dict)
                or not isinstance(methods, dict) or latest.get("integrity_level") != "strict"):
            return {}
        if isinstance(created, bool) or not isinstance(created, int):
            return {}
        age = time.time() - created
        if age < -300 or age > HISTORICAL_SNAPSHOT_MAX_AGE_DAYS * 86400:
            return {}
        if not _snapshot_methods_valid(snapshots, methods, "strict"):
            return {}
        return snapshots
    except Exception:
        return {}
...
def _anchor_guard_problems(anchor: dict, new_outputs: dict) -> list[str]:
    if not _guard_anchor_valid(anchor):
        return ["[P0-2] 累计删除基线无效"]
    new_anchor = _make_guard_anchor(new_outputs, anchor["created"])
    old_fingerprints = set(anchor["fingerprints"])
    new_fingerprints = set(new_anchor["fingerprints"])
    removed = len(old_fingerprints - new_fingerprints)
    if removed / len(old_fingerprints) > MAX_CUMULATIVE_DELETE_RATIO:
        return [f"[P0-2] 永久基线累计内容替换 {removed}/{len(old_fingerprints)} 超 "
                f"{MAX_CUMULATIVE_DELETE_RATIO:.0%}，拒绝覆盖"]
    return []


def _integrity_state(total_sources: int, fresh_sources: int, stale_sources: int,
                     failed_sources: int) -> tuple[str, list[str]]:
    reasons = []
    if fresh_sources != total_sources:
        reasons.append(f"fresh {fresh_sources}/{total_sources}")
    if stale_sources:
        reasons.append(f"stale {stale_sources}")
    if failed_sources:
        reasons.append(f"failed {failed_sources}")
    return ("strict", []) if not reasons else ("degraded", reasons)
...
def _recover_stale_backup():
    ...
    pending_path = os.path.join("dist", ".txn-pending")
    pending_swap = os.path.isfile(pending_path)
    output_valid = _output_dir_valid("dist")
    if pending_swap and output_valid:
        try:
            with open(pending_path, encoding="utf-8") as f:
                transaction_id = f.read().strip()
            report = load_json_file(os.path.join(REPORT_DIR, "latest.json"))
            committed = (
                re.fullmatch(r"[0-9a-f]{32}", transaction_id)
                and report.get("transaction_id") == transaction_id
                and report.get("aborted_reason") is None
                and report.get("output_sha256") == _dir_hash("dist")
            )
            if committed:
            ...
            pass
    dist_valid = output_valid and not pending_swap
    ...
    if valid_backups and not dist_valid:
        ...
        state = "产物本身有效" if output_valid else "产物也未通过校验"
        raise RuntimeError(
            f"检测到未完成事务且没有有效备份（{state}）；已保留 dist 供人工恢复")
    if dist_valid:
        for bak in backups:
...
def main():
    ...
    def _early_abort(reason):
        ...
        persisted = _write_report_or_stderr(os.path.join(REPORT_DIR, "latest.json"), report)
        target = f"{REPORT_DIR}/latest.json" if persisted else "stderr"
        print(f"⛔ 已中止并输出报告（{target}）：{reason}")
        raise SystemExit(reason)

    try:
        old_dist_valid = _recover_stale_backup()
        old_dist_hash = _dir_hash("dist") if old_dist_valid else None
    except Exception as exc:
    ...
    def _save_abort_report(reason):
        ...
        report = {
            ...
            "actual_dist_sha256": _safe_dist_hash(),
            ...
            "snapshot_trust_summary": snapshot_trust_summary,
            "snapshot_resolution_window_s": round(snapshot_resolution_window, 3),
    ...
    snapshot_resolution_finished = time.time()
    snapshot_resolution_window = snapshot_resolution_finished - snapshot_resolution_started
    if snapshot_resolution_window > MAX_SNAPSHOT_RESOLUTION_WINDOW:
        _early_abort(f"GitHub snapshot 解析窗口过长: {snapshot_resolution_window:.1f}s")
    unresolved_snapshots = [c for c, st in sha_status.items() if st.startswith("snapshot-unresolved:")]
    history_fallbacks = []
    if unresolved_snapshots:
        previous_snapshots = _load_source_snapshots("dist", old_dist_hash)
        for canon in list(unresolved_snapshots):
            previous_sha = previous_snapshots.get(canon)
            gr = parse_github_raw(canon)
            if gr and isinstance(previous_sha, str) and re.fullmatch(r"[0-9a-f]{40}", previous_sha):
                sha_map[canon] = GitHubRef(gr.owner, gr.repo, previous_sha, gr.path).raw_url()
                sha_status[canon] = "resolved-history"
                history_fallbacks.append(canon)
        unresolved_snapshots = [c for c in unresolved_snapshots
                                if sha_status[c].startswith("snapshot-unresolved:")]
    if unresolved_snapshots:
        details = [(c, sha_status[c]) for c in unresolved_snapshots[:3]]
        _early_abort(f"GitHub snapshot 解析失败（{len(unresolved_snapshots)}）：{details}")
    ref_service_fallbacks = [c for c, st in sha_status.items() if st == "resolved-ref-service"]
    if ref_service_fallbacks:
        print(f"⚠️ GitHub API 不可用，已由固定 IP ref service 解析 SHA（{len(ref_service_fallbacks)} 个源）")
    if history_fallbacks:
        print(f"⚠️ GitHub ref 无法解析，使用已验证历史 SHA（{len(history_fallbacks)} 个源）")
    snapshot_shas = {}
    snapshot_refs = {}
    for canon, pinned_url in sha_map.items():
        original_ref = parse_github_raw(canon)
        if original_ref:
            pinned_ref = parse_github_raw(pinned_url)
            if (not pinned_ref or not re.fullmatch(r"[0-9a-f]{40}", pinned_ref.ref)
                    or not original_ref.same_resource(pinned_ref, include_ref=False)):
                _early_abort(f"GitHub snapshot URL 非不可变 commit 或资源身份不匹配: {canon}")
            snapshot_shas[canon] = pinned_ref.ref
            snapshot_refs[canon] = pinned_ref
    snapshot_trust_summary = {}
    for method in sha_status.values():
        snapshot_trust_summary[method] = snapshot_trust_summary.get(method, 0) + 1
    snapshot_integrity_level = _snapshot_integrity_from_methods(snapshot_shas, sha_status)
    if snapshot_integrity_level == "invalid":
        _early_abort("GitHub snapshot method 状态无效")
    if snapshot_integrity_level == "degraded":
        os.makedirs(REPORT_DIR, exist_ok=True)
        report = {
            "ts": time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime()),
            "duration_s": round(time.time() - start, 1),
            "aborted_reason": None,
            "skipped_reason": "historical snapshot 仅允许复用旧 dist，不执行抓取或构建",
            "integrity_level": "degraded",
            "integrity_reasons": [f"historical-snapshot {len(history_fallbacks)}"],
            "snapshot_status": sha_status,
            "snapshot_shas": snapshot_shas,
            "snapshot_trust_summary": snapshot_trust_summary,
            "snapshot_resolution_window_s": round(snapshot_resolution_window, 3),
            "changed": False,
            "actual_dist_sha256": old_dist_hash,
        }
        _write_report_or_stderr(os.path.join(REPORT_DIR, "latest.json"), report)
        print("历史 SHA fallback：跳过抓取、构建和提交，保留旧 dist")
        return
    ...
    input_budget = _InputBudget(MAX_TOTAL_INPUT_SIZE)

    def fetch_batch(batch: list[str]):
        nonlocal cache_budget_exceeded, fetch_budget_exceeded
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {}
            for canon in batch:
                expected_ref = snapshot_refs.get(canon)
                expected_sha = expected_ref.ref if expected_ref is not None else None
                cache_meta = (load_cache(canon, old_dist_hash, expected_sha)
                              if old_dist_hash is not None else None)
                if cache_meta and cache_meta.get("data"):
                    if not input_budget.reserve_cache(len(cache_meta["data"])):
                        cache_meta = None
                        cache_budget_exceeded = True
                cache_metas[canon] = cache_meta
                pinned = sha_map.get(canon)
                if expected_ref is not None:
                    urls = _github_snapshot_urls(canon, pinned)
                else:
                    urls = [canon]
                    for url in groups[canon]:
                        if url not in urls:
                            urls.append(url)
                future = executor.submit(
                    fetch_group_worker, canon, urls, cache_meta, expected_ref, input_budget)
                futures[future] = canon
            for future in as_completed(futures):
                canon = futures[future]
                try:
                ...
                if data is not None:
                    ...
                    failed[canon] = meta
                    if (meta or {}).get("error") in {
                            "total-input-too-large", "decoder-exceeded-reserved-input"}:
                        fetch_budget_exceeded = True

    fetch_batch(canons_sorted)
    cache_bytes, fresh_bytes = input_budget.totals()
    if cache_budget_exceeded:
        print(f"⚠️ 缓存与网络输入累计接近 {MAX_TOTAL_INPUT_SIZE} 字节，超出缓存已忽略")
    if fetch_budget_exceeded:
        _abort(f"缓存与网络输入累计超过 {MAX_TOTAL_INPUT_SIZE} 字节，拒绝继续解析")
    ...
    for canon in canons_sorted:
        if canon in ok:
            ...
            if chosen == "fresh":
                ...
                pending_cache_updates.append(("bind", canon, None, None, None))
    ...
    integrity_level, integrity_reasons = _integrity_state(
        len(groups), len(fresh_valid_canons), len(stale_valid_canons), len(failed))
    if snapshot_integrity_level == "degraded":
        integrity_level = "degraded"
        integrity_reasons.append(f"historical-snapshot {len(history_fallbacks)}")
    print(f"完整性等级: {integrity_level}" + (f"（{', '.join(integrity_reasons)}）"
                                            if integrity_reasons else ""))
    ...
    newdir = os.path.basename(tempfile.mkdtemp(prefix="dist.new-", dir="."))
    try:
    ...
    try:
        ...
        old_hash = old_dist_hash
        new_hash = _dir_hash(newdir)
    except Exception as exc:
        _remove_path(newdir)
        _abort(f"产物写后校验失败: {type(exc).__name__}: {exc}")
    swap_backup = None
    swap_old_existed = False
    swap_completed = False
    content_changed = old_hash is None or old_hash != new_hash
    if integrity_level != "strict" and content_changed:
        _remove_path(newdir)
        _abort(f"完整性等级 {integrity_level} 不允许修改产物: {integrity_reasons}")
    if integrity_level != "strict":
        changed_any = False
        _remove_path(newdir)
        print("完整性降级但产物内容不变，禁止提交并保留旧元数据")
    else:
        changed_any = not (
            old_hash and old_hash == new_hash
            and _metadata_ok("dist", old_hash, new_counts, snapshot_shas))
    if integrity_level == "strict" and not changed_any:
        shutil.rmtree(newdir, ignore_errors=True)
        print("产物无变化，跳过提交")
    elif integrity_level == "strict":
        ...
        try:
            ...
            with open(os.path.join(newdir, "latest.json"), "w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "source_snapshots": snapshot_shas,
                    "source_snapshots_created": int(time.time()),
                    "source_snapshot_methods": sha_status,
                    "integrity_level": "strict",
                }, f, ensure_ascii=False, indent=1, allow_nan=False)
            with open(os.path.join(newdir, ".txn-pending"), "w", encoding="utf-8") as f:
            ...
            if old_existed and os.path.lexists(bak) and not os.path.lexists("dist"):
                try:
                    ...
                    recovery_errors.append(
                        f"旧产物回滚失败: {type(recovery_exc).__name__}: {recovery_exc}")
            if os.path.lexists(newdir):
                try:
                    _remove_path(newdir)
                except Exception as cleanup_exc:
                    recovery_errors.append(
                        f"新产物清理失败: {type(cleanup_exc).__name__}: {cleanup_exc}")
            detail = f"产物原子 swap 失败: {type(exc).__name__}: {exc}"
            if recovery_errors:
                detail += "；" + "；".join(recovery_errors)
            _abort(detail)
        swap_backup = bak if old_existed else None
        swap_old_existed = old_existed
        swap_completed = True
        print("产物目录已交换，等待 cache 与 report 提交")
    if os.path.lexists(newdir):
        _remove_path(newdir)
    try:
        cache_rollback_state = _snapshot_cache_state(
            canon for _, canon, _, _, _ in pending_cache_updates)
    except Exception as exc:
        dist_rollback_ok = True
        if swap_completed:
            dist_rollback_ok = _rollback_dist_swap(swap_backup, swap_old_existed)
            swap_completed = False
        reason = f"cache 事务备份失败：{type(exc).__name__}: {exc}"
        if not dist_rollback_ok:
            reason += "；dist 回滚未完成，备份保留供下次启动恢复"
        _abort(reason)
    cache_update_failures = []
    cache_mutated = False
    for action, canon, data, meta, validator_url in pending_cache_updates:
        snapshot_sha = snapshot_shas.get(canon)
        if action == "save":
            cache_ok = save_cache(canon, data, meta, validator_url, new_hash, snapshot_sha)
        elif action == "touch":
            cache_ok = touch_cache(canon, new_hash, snapshot_sha)
        else:
            cache_ok = bind_cache(canon, new_hash, snapshot_sha)
        cache_mutated = True
        if not cache_ok:
            cache_update_failures.append({"canon": canon, "action": action})
    if cache_update_failures:
        cache_rollback_ok = _restore_cache_state(cache_rollback_state)
        dist_rollback_ok = True
        if swap_completed:
            dist_rollback_ok = _rollback_dist_swap(swap_backup, swap_old_existed)
            swap_completed = False
        reason = f"cache 事务失败：{cache_update_failures[:5]}"
        if not cache_rollback_ok:
            reason += "；cache 回滚未完成"
        if not dist_rollback_ok:
            reason += "；dist 回滚未完成，备份保留供下次启动恢复"
        _abort(reason)
    ...
    report = {
        ...
        "client_url_network_policy": {
            "dns_enforcement": "client-required",
            "literal_private_rejected": True,
            "reserved_internal_suffix_rejected": True,
        },
        "fetched": {
    ...
    try:
        _write_json_atomic(os.path.join(REPORT_DIR, "latest.json"), report)
    except Exception as exc:
        cache_rollback_status = "not-required"
        if cache_mutated:
            cache_rollback_status = (
                "complete" if _restore_cache_state(cache_rollback_state) else "incomplete")
        dist_rollback_status = "not-required"
        if swap_completed:
            dist_rollback_status = ("complete" if _rollback_dist_swap(
                swap_backup, swap_old_existed) else "incomplete")
            swap_completed = False
        failure = {
            "transaction_status": "aborted",
            "phase": "report-write",
            "report_write_error": f"{type(exc).__name__}: {exc}",
            "cache_rollback_status": cache_rollback_status,
            "dist_rollback_status": dist_rollback_status,
            "transaction_id": transaction_id,
            "actual_dist_sha256": _safe_dist_hash(),
        }
    ...
    if swap_completed:
        try:
            os.remove(os.path.join("dist", ".txn-pending"))
        except Exception as exc:
            cache_rollback_ok = (
                _restore_cache_state(cache_rollback_state) if cache_mutated else True)
            dist_rollback_ok = _rollback_dist_swap(swap_backup, swap_old_existed)
            swap_completed = False
            reason = f"事务提交标记清理失败: {type(exc).__name__}: {exc}"
            if not cache_rollback_ok:
                reason += "；cache 回滚未完成"
            if not dist_rollback_ok:
                reason += "；dist 回滚未完成，备份保留供下次启动恢复"
            _abort(reason)
        if swap_backup and os.path.lexists(swap_backup):
            try:
                _remove_path(swap_backup)
            except Exception as exc:
                print(f"⚠️ 旧备份清理失败，将由下次运行恢复: {exc}")
        print("产物、cache 与 report 事务提交完成")
...
def validate_outputs(outputs: dict[str, list]) -> tuple[list[str], str]:
    ...
    for fn in OUTPUT_FILES:
        ...
        if not isinstance(ms, list):
            ...
            for index, item in enumerate(ms):
                if not isinstance(item, dict):
                    problems.append(f"{fn}[{index}] 非对象")
                    bad_types = True
    if bad_types:
        return problems, "n/a"
    ...
    regex_problem_prefixes = {
        "java": "Java 正则校验失败",
        "limit": "正则资源限制",
        "unavailable": "Java 正则校验不可用",
    }
    prefix = regex_problem_prefixes.get(jmode)
    if prefix is None:
        problems.append(f"正则校验器返回非法模式: {jmode!r}")
    else:
        for failure in jfails:
            problems.append(f"{prefix}: {failure[:120]}")
...
def run_selftests():
    ...
    def write_valid_output_dir(directory, label):
        ...
        for fn, media_sources in values.items():
            _write_json_limited(
                ...
                {"exportedMediaSourceDataList": {"mediaSources": media_sources}},
                MAX_OUTPUT_FILE_SIZE,
            )
    ...
    class T(unittest.TestCase):
        ...
        def test_dependency_version_parser(self):
            self.assertEqual(_dependency_version("2.34.2"), (2, 34, 2))
            self.assertEqual(_dependency_version("2.7.0.dev1"), (2, 7, 0))
            self.assertIsNone(_dependency_version("2.34"))
        ...
        def test_unsupported_scheme_and_https_policy_are_distinct(self):
            self.assertTrue(has_unsupported_scheme("file:///etc/passwd"))
            self.assertFalse(has_unsupported_scheme("http://example.com/source.json"))
            with self.assertRaisesRegex(ValueError, "非法协议"):
        ...
        def test_github_path_traversal_rejected_at_all_entrypoints(self):
            sha = "a" * 40
            malicious = (
                f"https://raw.githubusercontent.com/o/r/{sha}/../attacker/x.json",
                f"https://raw.githubusercontent.com/o/r/{sha}/%2e%2e/attacker/x.json",
                f"https://github.com/o/r/raw/{sha}/.%2e/attacker/x.json",
                f"https://cdn.jsdelivr.net/gh/o/r@{sha}/%252e%252e/attacker/x.json",
                f"https://gh-proxy.com/raw.githubusercontent.com/o/r/{sha}/../attacker/x.json",
            )
            for url in malicious:
                with self.subTest(url=url), self.assertRaises(ValueError):
                    normalize(url)

        def test_snapshot_identity_binds_owner_repo_ref_and_path(self):
            sha = "b" * 40
            expected = GitHubRef("owner", "repo", sha, "dir/x.json")
            allowed = (
                f"https://raw.githubusercontent.com/OWNER/REPO/{sha}/dir/x.json",
                f"https://cdn.jsdelivr.net/gh/owner/repo@{sha}/dir/x.json",
            )
            rejected = (
                f"https://raw.githubusercontent.com/other/repo/{sha}/dir/x.json",
                f"https://raw.githubusercontent.com/owner/other/{sha}/dir/x.json",
                f"https://raw.githubusercontent.com/owner/repo/{sha}/dir/y.json",
                f"https://raw.githubusercontent.com/owner/repo/{'c' * 40}/dir/x.json",
                f"https://gh-proxy.com/raw.githubusercontent.com/owner/repo/{sha}/dir/x.json",
            )
            self.assertTrue(all(_url_matches_snapshot(url, expected) for url in allowed))
            self.assertFalse(any(_url_matches_snapshot(url, expected) for url in rejected))
            with self.assertRaises(ValueError):
                _github_snapshot_urls(
                    "https://raw.githubusercontent.com/owner/repo/main/dir/x.json",
                    f"https://raw.githubusercontent.com/other/repo/{sha}/dir/x.json")

        @mock.patch.object(requests.Session, "get")
        def test_snapshot_redirect_must_preserve_commit(self, mget):
            sha = "a" * 40
            start = f"https://raw.githubusercontent.com/o/r/{sha}/x.json"
            mutable = "https://raw.githubusercontent.com/o/r/main/x.json"
            expected = GitHubRef("o", "r", sha, "x.json")
            mget.return_value = self._resp(302, b"", headers={"Location": mutable})
            data, meta = fetch_url(start, None, time.monotonic() + 30, expected)
            self.assertIsNone(data)
            self.assertEqual(meta["error"], "snapshot-redirect-mismatch")
            self.assertEqual(mget.call_count, 1)

        @mock.patch.object(requests.Session, "get")
        def test_snapshot_redirect_same_commit_allowed(self, mget):
            sha = "b" * 40
            start = f"https://raw.githubusercontent.com/o/r/{sha}/x.json"
            mirror = f"https://cdn.jsdelivr.net/gh/o/r@{sha}/x.json"
            expected = GitHubRef("o", "r", sha, "x.json")
            mget.side_effect = [self._resp(302, b"", headers={"Location": mirror}),
                                self._resp(200, b'{}')]
            data, meta = fetch_url(start, None, time.monotonic() + 30, expected)
        ...
        def test_cache_bound_to_accepted_output(self):
            import tempfile
            td = tempfile.mkdtemp()
            old = CACHE_DIR
            globals()["CACHE_DIR"] = td
            ...
            second_hash = "b" * 64
            first_snapshot = "c" * 40
            second_snapshot = "d" * 40
            try:
                save_cache(canon, b'{"a":1}', {}, canon, first_hash, first_snapshot)
                self.assertIsNotNone(load_cache(canon, first_hash, first_snapshot))
                self.assertIsNone(load_cache(canon, second_hash, first_snapshot))
                self.assertIsNone(load_cache(canon, first_hash, second_snapshot))
                before_ts = load_cache(canon)["ts"]
                bind_cache(canon, second_hash, second_snapshot)
                rebound = load_cache(canon, second_hash, second_snapshot)
                self.assertIsNotNone(rebound)
                self.assertEqual(rebound["ts"], before_ts)
                self.assertEqual(rebound["accepted_output_sha256"], second_hash)
                self.assertEqual(rebound["snapshot_sha"], second_snapshot)
            finally:
                globals()["CACHE_DIR"] = old
                shutil.rmtree(td, ignore_errors=True)

        def test_cache_transaction_snapshot_restores_all_files(self):
            td = tempfile.mkdtemp()
            old_dir = CACHE_DIR
            globals()["CACHE_DIR"] = td
            first = "https://example.com/first.json"
            second = "https://example.com/second.json"
            try:
                self.assertTrue(save_cache(first, b"old", {}, first, "a" * 64))
                before = open(cache_path(first), "rb").read()
                state = _snapshot_cache_state([first, second])
                self.assertTrue(save_cache(first, b"new", {}, first, "b" * 64))
                self.assertTrue(save_cache(second, b"created", {}, second, "b" * 64))
                self.assertTrue(_restore_cache_state(state))
                self.assertEqual(open(cache_path(first), "rb").read(), before)
                self.assertFalse(os.path.exists(cache_path(second)))
            finally:
                globals()["CACHE_DIR"] = old_dir
                shutil.rmtree(td, ignore_errors=True)

        @unittest.skipUnless(hasattr(os, "symlink"), "需要 symlink")
        def test_cache_directory_symlink_rejected(self):
            td = tempfile.mkdtemp()
            old_dir = CACHE_DIR
            target = os.path.join(td, "target")
            link = os.path.join(td, "cache-link")
            os.makedirs(target)
            os.symlink(target, link)
            globals()["CACHE_DIR"] = link
            try:
                canon = "https://example.com/x.json"
                self.assertFalse(save_cache(canon, b"data", {}, canon))
        ...
        def test_try_parse_bad_item_does_not_crash(self):
            ...
            ms.extend(copy.deepcopy(ms[2]) for _ in range(7))
        ...
        def test_group_worker_tries_mirror_after_invalid_200(self):
            canon = "https://raw.githubusercontent.com/a/b/main/x.json"
            invalid = b'{"not":"a source"}'
            item = {"factoryId": "web-selector", "version": 2,
                    "arguments": {"name": "X", "description": "", "iconUrl": "",
                                  "searchConfig": {"searchUrl": "https://x.com"}}}
            valid = json.dumps({
                "exportedMediaSourceDataList": {"mediaSources": [item]}}).encode()
            with mock.patch(__name__ + ".fetch_url", side_effect=[
                    (invalid, {"status": 200, "error": None}),
                    (valid, {"status": 200, "error": None})]) as fetcher:
                data, meta = fetch_group_worker(
                    canon, ["https://first.example/x", "https://second.example/x"], None)
            self.assertEqual(data, valid)
            self.assertEqual(fetcher.call_count, 2)
            self.assertEqual(meta["previous_attempts"][0]["error"], "fresh-content-invalid")
        ...
        def test_save_cache_failure_reported_and_cleans_tmp(self):
            import tempfile
            td = tempfile.mkdtemp()
            old = CACHE_DIR
            globals()["CACHE_DIR"] = td
            ...
            try:
                ...
                with mock.patch.object(os, "replace", side_effect=OSError("injected")):
                    result = save_cache(canon, b'{"a":1}', {}, canon)
        ...
        def test_touch_cache_replace_failure_cleans_tmp(self):
            import tempfile
            td = tempfile.mkdtemp()
            old = CACHE_DIR
            globals()["CACHE_DIR"] = td
            ...
            try:
                ...
                p = cache_path(canon)
                before = load_json_file(p)
                with mock.patch.object(os, "replace", side_effect=OSError("injected")):
                    result = touch_cache(canon)
                self.assertFalse(result)
                self.assertFalse(os.path.lexists(p + ".tmp"))
                after = load_json_file(p)
        ...
        def test_named_group_conversion_ignores_literals(self):
            ...
            converted = normalize_regex_named_groups(r"(?P<word>a+)-(?P=word)")
            self.assertEqual(converted, r"(?<word>a+)-\k<word>")
            self.assertEqual(java_check_regexes([converted])[0], [])
            self.assertEqual(normalize_regex_named_groups(r"\(?P<literal>"), r"\(?P<literal>")
            self.assertEqual(normalize_regex_named_groups(r"\(?P=literal)"), r"\(?P=literal)")
            self.assertEqual(normalize_regex_named_groups(r"[(?P<literal>(?P=literal)]"),
        ...
        def test_pinned_session_always_ignores_env_proxy(self):
            old = getattr(_pinned_tls, "sessions", None)
            _pinned_tls.sessions = {}
            try:
                with mock.patch.dict(os.environ, {
                        "ALLOW_ENV_PROXY": "1", "HTTPS_PROXY": "http://proxy.invalid"},
                        clear=True):
        ...
        def test_validate_non_object_items_fails_without_traceback(self):
            d = tempfile.mkdtemp()
            try:
                os.makedirs(os.path.join(d, "dist"))
                for filename in OUTPUT_FILES:
                    with open(os.path.join(d, "dist", filename), "w", encoding="utf-8") as handle:
                        json.dump({"exportedMediaSourceDataList": {"mediaSources": [None]}}, handle)
                result = subprocess.run(
                    [sys.executable, os.path.abspath(__file__), "--validate"], cwd=d,
                    capture_output=True, text=True, check=False)
                self.assertEqual(result.returncode, 1)
                self.assertIn("非对象", result.stdout)
                self.assertNotIn("Traceback", result.stderr)
            finally:
                shutil.rmtree(d, ignore_errors=True)

        def test_unknown_cli_argument_rejected(self):
            for arguments in (("--unknown",), ("--validate", "extra"), ("--test", "extra")):
                result = subprocess.run(
        ...
        def test_snapshot_method_trust_levels(self):
            snapshots = {"canon": "a" * 40}
            history = {"canon": "resolved-history"}
            self.assertTrue(_snapshot_methods_valid(snapshots, history, "degraded"))
            self.assertFalse(_snapshot_methods_valid(snapshots, history, "strict"))
            strict = {"canon": "resolved-ref-service"}
            self.assertTrue(_snapshot_methods_valid(snapshots, strict, "strict"))
            self.assertFalse(_snapshot_methods_valid(snapshots, strict, "degraded"))
        ...
        def test_metadata_validation(self):
            ...
            try:
                ...
                anchor = {"created": int(time.time()), "fingerprints": ["a" * 64]}
                ...
                with open(os.path.join(d, "latest.json"), "w", encoding="utf-8") as f:
                    json.dump({"output_sha256": "abc", "counts": counts,
                               "guard_anchor": anchor,
                               "source_snapshots": {"canon": "b" * 40},
                               "source_snapshots_created": int(time.time()),
                               "source_snapshot_methods": {"canon": "resolved-api"},
                               "integrity_level": "strict"}, f)
                self.assertTrue(_metadata_ok(d, "abc", counts))
                self.assertTrue(_metadata_ok(d, "abc", counts, {"canon": "b" * 40}))
                self.assertFalse(_metadata_ok(d, "abc", counts, {"canon": "c" * 40}))
                self.assertFalse(_metadata_ok(d, "wrong", counts))
                with open(os.path.join(d, "latest.json"), "w", encoding="utf-8") as f:
                    json.dump({"output_sha256": "abc", "counts": counts,
                               "source_base_commit": "not-a-sha", "guard_anchor": anchor,
                               "source_snapshots": {"canon": "b" * 40},
                               "source_snapshots_created": int(time.time()),
                               "source_snapshot_methods": {"canon": "resolved-api"},
                               "integrity_level": "strict"}, f)
        ...
        def test_historical_snapshot_expiry(self):
            import tempfile
            d = tempfile.mkdtemp()
            try:
                _write_json_atomic(os.path.join(d, "latest.json"), {
                    "output_sha256": "a" * 64,
                    "source_snapshots": {"canon": "b" * 40},
                    "source_snapshots_created":
                        int(time.time()) - (HISTORICAL_SNAPSHOT_MAX_AGE_DAYS + 1) * 86400,
                })
                self.assertEqual(_load_source_snapshots(d, "a" * 64), {})
            finally:
                shutil.rmtree(d, ignore_errors=True)
        ...
        def test_new_item_source_concentration_guard(self):
            def mk(i):
                return {"factoryId": "web-selector", "version": 2,
                        "arguments": {"name": f"C{i}", "description": "", "iconUrl": "",
                                      "searchConfig": {"searchUrl": f"https://c{i}.com"}}}
            old = [mk(i) for i in range(100)]
            new = old + [mk(i) for i in range(100, 120)]
            origins = {}
            for i in range(100, 120):
                key = ("web-selector", f"C{i}", f"https://c{i}.com")
                origins[key] = "source-a" if i < 113 else f"source-{i}"
            problems = _concentration_guard_problems(
                {"all.json": old}, {"all.json": new}, origins)
            self.assertTrue(any("过度集中" in problem for problem in problems))
            for i in range(112, 113):
                key = ("web-selector", f"C{i}", f"https://c{i}.com")
                origins[key] = f"source-{i}"
            self.assertFalse(_concentration_guard_problems(
                {"all.json": old}, {"all.json": new}, origins))

        def test_cumulative_content_replacement_guard(self):
            def mk(i):
                return {"factoryId": "web-selector", "version": 2,
                        "arguments": {"name": f"A{i}", "description": "", "iconUrl": "",
                                      "searchConfig": {"searchUrl": f"https://a{i}.com"}}}
            original = [mk(i) for i in range(100)]
            anchor = _make_guard_anchor({"all.json": original})
            ten = copy.deepcopy(original)
            for item in ten[:10]:
                item["arguments"]["description"] = "changed"
            self.assertFalse(_anchor_guard_problems(anchor, {"all.json": ten}))
            eleven = copy.deepcopy(original)
            for item in eleven[:11]:
                item["arguments"]["description"] = "changed"
            self.assertTrue(any("累计内容替换" in p
                                for p in _anchor_guard_problems(anchor, {"all.json": eleven})))
            permanent = _make_guard_anchor({"all.json": original},
                                           int(time.time()) - 3650 * 86400)
            self.assertTrue(_guard_anchor_valid(permanent))

        @mock.patch.object(requests, "Session")
        def test_github_api_always_ignores_env_proxy(self, session_cls):
            api = mock.MagicMock()
            ...
            with mock.patch.dict(os.environ, {
                    "ALLOW_ENV_PROXY": "1", "HTTPS_PROXY": "http://proxy.invalid"}), \
                 mock.patch(__name__ + "._resolve_ref_via_http",
                            return_value=("c" * 40, "resolved-ref-service")):
                _, status = resolve_commit_shas(["https://raw.githubusercontent.com/o/r/main/x.json"])
            self.assertFalse(api.trust_env)
            self.assertEqual(next(iter(status.values())), "resolved-ref-service")
        ...
        def test_github_api_invalid_sha_rejected(self, session_cls):
            ...
            response.content = b'{"sha":"not-a-sha"}'
            api.get.return_value = response
            session_cls.return_value.__enter__.return_value = api
            canon = "https://raw.githubusercontent.com/o/r/main/x.json"
            with mock.patch(__name__ + "._resolve_ref_via_http",
                            return_value=(None, "ref-service-ref-not-found")):
                out, status = resolve_commit_shas([canon])
            self.assertTrue(status[canon].startswith("snapshot-unresolved:api-invalid-response:"))
            self.assertEqual(out[canon], canon)

        def test_ref_service_fallback_uses_pinned_fetch_stack(self):
            payload = ("e" * 40 + " refs/heads/main\n").encode()
            advertisement = (b"001e# service=git-upload-pack\n0000"
                             + f"{len(payload) + 4:04x}".encode() + payload + b"0000")
            meta = {"redirects": [
                "https://github.com/Owner/Repo.git/info/refs?service=git-upload-pack"]}
            with mock.patch(__name__ + ".fetch_url", return_value=(advertisement, meta)) as fetcher:
                sha, status = _resolve_ref_via_http("Owner", "Repo", "main")
            self.assertEqual(sha, "e" * 40)
            self.assertEqual(status, "resolved-ref-service")
            self.assertTrue(fetcher.call_args.args[0].startswith("https://github.com/"))
            bad_meta = {"redirects": [
                "https://github.com/Other/Repo.git/info/refs?service=git-upload-pack"]}
            with mock.patch(__name__ + ".fetch_url", return_value=(advertisement, bad_meta)):
                sha, status = _resolve_ref_via_http("Owner", "Repo", "main")
            self.assertIsNone(sha)
            self.assertEqual(status, "ref-service-trust-boundary")

        def test_github_snapshot_urls_are_immutable(self):
            canon = "https://raw.githubusercontent.com/o/r/main/path/x.json"
            pinned = "https://raw.githubusercontent.com/o/r/" + "f" * 40 + "/path/x.json"
            urls = _github_snapshot_urls(canon, pinned)
            self.assertEqual(urls[0], pinned)
            self.assertIn("@" + "f" * 40 + "/", urls[1])
            self.assertNotIn("/main/", "\n".join(urls))

        @mock.patch.object(requests, "Session")
        def test_github_api_strict_json_rejected(self, session_cls):
            api = mock.MagicMock()
            response = mock.MagicMock()
            response.status_code = 200
            api.get.return_value = response
            session_cls.return_value.__enter__.return_value = api
            canon = "https://raw.githubusercontent.com/o/r/main/x.json"
            for body in (b'{"sha":"' + b'a' * 40 + b'","sha":"' + b'b' * 40 + b'"}',
                         b'{"sha":"' + b'a' * 40 + b'","x":NaN}',
                         b'{"sha":"' + b'a' * 40 + b'","x":1e9999}'):
                response.content = body
                with mock.patch(__name__ + "._resolve_ref_via_http",
                                return_value=(None, "ref-service-failed")):
                    _, status = resolve_commit_shas([canon])
                self.assertIn("api-invalid-json", status[canon])
        ...
        def test_github_api_case_insensitive_repo_dedup(self, session_cls):
            ...
            response = mock.MagicMock()
            response.status_code = 200
            response.content = json.dumps({"sha": "a" * 40}).encode()
            api.get.return_value = response
            session_cls.return_value.__enter__.return_value = api
            ...
            self.assertEqual(status[a], "resolved-api")
            self.assertEqual(status[b], "resolved-api")
        ...
        def test_abbreviated_sha_is_resolved(self, session_cls):
            ...
            response = mock.MagicMock()
            response.status_code = 200
            response.content = json.dumps({"sha": "b" * 40}).encode()
            api.get.return_value = response
            session_cls.return_value.__enter__.return_value = api
            canon = "https://raw.githubusercontent.com/o/r/abcdef1/x.json"
            out, status = resolve_commit_shas([canon])
            self.assertEqual(status[canon], "resolved-api")
            self.assertIn("/" + "b" * 40 + "/", out[canon])

        @mock.patch.object(requests, "Session")
        def test_all_refs_attempt_api_before_ref_service(self, session_cls):
            api = mock.MagicMock()
            response = mock.MagicMock()
            response.status_code = 404
            api.get.return_value = response
            ...
            with mock.patch(__name__ + "._resolve_ref_via_http",
                            return_value=("d" * 40, "resolved-ref-service")) as resolver:
                _, status = resolve_commit_shas(canons)
            self.assertEqual(session_cls.call_count, 3)
            self.assertEqual(resolver.call_count, 3)
            self.assertTrue(all(v == "resolved-ref-service" for v in status.values()))
        ...
        def test_malformed_export_wrapper_not_bypassed(self):
            ...
            with self.assertRaisesRegex(ValueError, "UTF-8 JSON 结构无效"):
                load_json_bytes(json.dumps(obj).encode())

        def test_utf8_json_errors_do_not_fall_through_to_gbk(self):
            invalid_structure = json.dumps({"x": "中文"}, ensure_ascii=False).encode("utf-8")
            with self.assertRaisesRegex(ValueError, "UTF-8 JSON 结构无效"):
                load_json_bytes(invalid_structure)
            with self.assertRaisesRegex(ValueError, "UTF-8 JSON 语法"):
                load_json_bytes(b'{"x":')

        def test_gbk_json_fallback_preserves_text(self):
            obj = {"exportedMediaSourceDataList": {"mediaSources": [{"name": "中文"}]}}
            encoded = json.dumps(obj, ensure_ascii=False).encode("gbk")
            self.assertEqual(load_json_bytes(encoded), obj)
        ...
        def test_source_item_valid_ratio(self):
            ...
            below = [good] * 89 + [None] * 11
            body = json.dumps({"exportedMediaSourceDataList": {"mediaSources": below}}).encode()
            self.assertFalse(try_parse(body, canon, [])[1])
            edge = [good] * 90 + [None] * 10
            body = json.dumps({"exportedMediaSourceDataList": {"mediaSources": edge}}).encode()
            seg, valid = try_parse(body, canon, [])
            self.assertTrue(valid)
            self.assertEqual(len(seg), 90)
        ...
        def test_total_output_size_limit(self):
            d = tempfile.mkdtemp()
            try:
                for filename in OUTPUT_FILES[:2]:
                    with open(os.path.join(d, filename), "wb") as handle:
                        handle.write(b"abcd")
                with mock.patch(__name__ + ".MAX_TOTAL_OUTPUT_SIZE", 7):
                    problems = _output_size_problems(d)
                self.assertTrue(any("总大小" in problem for problem in problems))
            finally:
                shutil.rmtree(d, ignore_errors=True)
        ...
        def test_recover_preserves_pending_dist_without_valid_backup(self):
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            try:
                os.chdir(d)
                write_valid_output_dir("dist", "pending")
                with open(os.path.join("dist", ".txn-pending"), "w", encoding="utf-8") as handle:
                    handle.write("invalid-transaction\n")
                with self.assertRaisesRegex(RuntimeError, "已保留 dist"):
                    _recover_stale_backup()
                self.assertTrue(_output_dir_valid("dist"))
                self.assertTrue(os.path.isfile(os.path.join("dist", ".txn-pending")))
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)

        def test_recover_finalizes_reported_transaction(self):
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            try:
                os.chdir(d)
                write_valid_output_dir("dist", "new")
                write_valid_output_dir("dist.bak-1", "old")
                transaction_id = "a" * 32
                with open(os.path.join("dist", ".txn-pending"), "w", encoding="utf-8") as f:
                    f.write(transaction_id + "\n")
                os.makedirs("reports")
                _write_json_atomic(os.path.join("reports", "latest.json"), {
                    "transaction_id": transaction_id,
                    "aborted_reason": None,
                    "output_sha256": _dir_hash("dist"),
                })
                with mock.patch("builtins.print"):
                    self.assertTrue(_recover_stale_backup())
                restored = extract(load_json_file(os.path.join("dist", "all.json")))
                self.assertEqual(restored[0]["arguments"]["name"], "Wnew")
                self.assertFalse(os.path.exists(os.path.join("dist", ".txn-pending")))
        ...
        def test_regex_validation_modes_have_reachable_messages(self):
            import tempfile
            d = tempfile.mkdtemp()
            try:
                write_valid_output_dir(d, "mode")
                outputs = {
                    fn: extract(load_json_file(os.path.join(d, fn)))
                    for fn in OUTPUT_FILES
                }
                cases = {
                    "java": "Java 正则校验失败",
                ...
                for mode, prefix in cases.items():
                    with self.subTest(mode=mode), mock.patch(
                            __name__ + ".java_check_regexes",
                            return_value=(["detail"], mode)):
                        problems, returned_mode = validate_outputs(outputs)
                    self.assertEqual(returned_mode, mode)
                    self.assertTrue(any(problem == f"{prefix}: detail" for problem in problems))
            finally:
                shutil.rmtree(d, ignore_errors=True)
        ...
        def test_non_global_addresses_rejected(self):
            ...
            for host in ("localhost", "metadata.service.internal", "printer.local", "router.lan"):
                self.assertTrue(is_private_hostname(host))
                self.assertEqual(url_shallow_ok(f"https://{host}/x")[1], "private-hostname")
        ...
        def test_known_search_config_field_types(self):
            ...
            self.assertFalse(validate_item(item)[0])
            for target in ("item", "arguments", "searchConfig"):
                item = copy.deepcopy(base)
                if target == "item":
                    item["unknown"] = 1
                elif target == "arguments":
                    item["arguments"]["unknown"] = 1
                else:
                    item["arguments"]["searchConfig"]["unknown"] = 1
                ok, issues = validate_item(item)
                self.assertFalse(ok)
                self.assertTrue(any("未知" in issue for issue in issues))

        def test_control_characters_rejected_in_all_string_fields(self):
            base = {"factoryId": "web-selector", "version": 2,
                    "arguments": {"name": "X", "description": "", "iconUrl": "",
                                  "searchConfig": {"searchUrl": "https://x.com"}}}
            for path in ("name", "description", "userAgent", "matchVideoUrl"):
                item = copy.deepcopy(base)
                if path in {"name", "description"}:
                    item["arguments"][path] = "bad\r\nvalue"
                else:
                    item["arguments"]["searchConfig"][path] = "bad\x00value"
                valid, issues = validate_item(item)
                self.assertFalse(valid, path)
                self.assertTrue(any("控制字符" in issue for issue in issues), path)

        def test_retry_after_huge_integer_safe(self):
            self.assertEqual(parse_retry_after("9" * (MAX_RETRY_AFTER_DIGITS + 1), 1.25), 1.25)
            self.assertEqual(parse_retry_after("9" * MAX_RETRY_AFTER_DIGITS, 1.25), 15)
        ...
        def test_public_dns_addresses_rotate(self):
            ...
            self.assertEqual(_choose_public_ip(host, infos), "93.184.216.34")
            changed_infos = [(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("8.8.8.8", 0))]
            self.assertEqual(_choose_public_ip(host, changed_infos), "8.8.8.8")
            self.assertEqual(_choose_public_ip(host, infos), "93.184.216.34")

        def test_dns_resolution_respects_deadline(self):
            worker = mock.MagicMock()
            worker.query.side_effect = TimeoutError("blocked")
            pool = queue.LifoQueue(maxsize=1)
            pool.put(worker)
            with mock.patch(__name__ + "._get_dns_pool", return_value=pool):
                started = time.monotonic()
        ...
        def test_dns_resolver_environment_is_minimal(self):
            with mock.patch.dict(os.environ, {
                "GITHUB_TOKEN": "secret",
                ...
                resolver_env = _resolver_environment()
            self.assertNotIn("GITHUB_TOKEN", resolver_env)
            self.assertNotIn("HTTPS_PROXY", resolver_env)
            self.assertNotIn("HTTP_PROXY", resolver_env)
            self.assertEqual(resolver_env["PYTHONUTF8"], "1")

        def test_dns_resolver_command_uses_fixed_script_entrypoint(self):
            command = _resolver_command()
            self.assertEqual(command[:3], [sys.executable, "-I", "-u"])
            self.assertEqual(os.path.realpath(command[3]), os.path.realpath(__file__))
            self.assertEqual(command[4:], ["--dns-resolver-worker"])
            self.assertNotIn("-c", command)

        def test_dns_resolver_worker_rejects_overlong_line(self):
            request = json.dumps("a" * DNS_MAX_QUERY_LINE) + "\n"
            result = subprocess.run(
                _resolver_command(), input=request, capture_output=True, text=True,
                timeout=5, check=False, env=_resolver_environment())
            self.assertEqual(result.returncode, 0)
            self.assertEqual(json.loads(result.stdout), {"error": "RequestTooLong"})

        def test_dns_worker_runs_without_site_packages(self):
            script = os.path.realpath(os.path.abspath(__file__))
            command = [
                sys.executable, "-I", "-S", "-u", script, _DNS_RESOLVER_WORKER_ARG]
            request = json.dumps("a" * DNS_MAX_QUERY_LINE) + "\n"
            result = subprocess.run(
            ...
            self.assertEqual(json.loads(result.stdout), {"error": "RequestTooLong"})
            self.assertNotIn("缺少依赖", result.stderr)

        def test_dns_process_worker_restarts_after_timeout(self):
            import tempfile
            td = tempfile.mkdtemp()
            blocking_path = os.path.join(td, "blocking_worker.py")
            responsive_path = os.path.join(td, "responsive_worker.py")
            with open(blocking_path, "w", encoding="utf-8") as f:
                f.write("import sys,time\nfor line in sys.stdin:\n time.sleep(10)\n")
            with open(responsive_path, "w", encoding="utf-8") as f:
                f.write(
                    "import json,sys\nfor line in sys.stdin:\n"
                    " print(json.dumps({'addresses':['93.184.216.34']}),flush=True)\n"
                )
            blocking_command = [sys.executable, "-I", "-u", blocking_path]
            responsive_command = [sys.executable, "-I", "-u", responsive_path]
            worker = None
            try:
                with mock.patch(__name__ + "._resolver_command", return_value=blocking_command):
                    worker = _DNSResolverWorker()
                    old_proc = worker.proc
                    pool = queue.LifoQueue(maxsize=1)
                    pool.put(worker)
                    with mock.patch(__name__ + "._get_dns_pool", return_value=pool):
                        infos, error = _bounded_getaddrinfo(
                            "blocked.example", time.monotonic() + 0.05)
                    self.assertIsNone(infos)
                    self.assertEqual(error, "dns-timeout")
                    self.assertIsNot(worker.proc, old_proc)
                    self.assertIsNotNone(old_proc.poll())
                with mock.patch(__name__ + "._resolver_command", return_value=responsive_command):
                    worker.start()
                    with mock.patch(__name__ + "._get_dns_pool", return_value=pool):
                        infos, error = _bounded_getaddrinfo(
                            "recovered.example", time.monotonic() + 1)
        ...
        def test_dns_workers_recover_after_full_saturation(self):
            ...
            class BlockingWorker:
                ...
                def query(self, _host, _timeout):
                    nonlocal entered
                    if self.recovered:
                        return ["93.184.216.34"], None
                    with entered_lock:
                        entered += 1
                        if entered == DNS_MAX_OUTSTANDING:
                            all_entered.set()
                    release.wait(2)
            ...
            workers = [BlockingWorker() for _ in range(DNS_MAX_OUTSTANDING)]
            pool = queue.LifoQueue(maxsize=DNS_MAX_OUTSTANDING)
            for worker in workers:
                pool.put(worker)
            with mock.patch(__name__ + "._get_dns_pool", return_value=pool):
                with ThreadPoolExecutor(max_workers=DNS_MAX_OUTSTANDING) as ex:
                    futures = [ex.submit(_bounded_getaddrinfo, f"h{i}.example",
                                         time.monotonic() + 1)
                               for i in range(DNS_MAX_OUTSTANDING)]
                    self.assertTrue(all_entered.wait(1))
                    infos, error = _bounded_getaddrinfo("overflow.example",
                                                       time.monotonic() + 0.01)
                    self.assertIsNone(infos)
                    self.assertEqual(error, "dns-timeout")
                    release.set()
                    self.assertTrue(all(f.result()[1] == "dns-timeout" for f in futures))
                self.assertTrue(all(worker.restarts == 1 for worker in workers))
                for worker in workers:
                    worker.recovered = True
                infos, error = _bounded_getaddrinfo("recovered.example", time.monotonic() + 1)
            self.assertIsNone(error)
            self.assertEqual(infos[0][4][0], "93.184.216.34")
        ...
        def test_integrity_levels(self):
            self.assertEqual(_integrity_state(10, 10, 0, 0), ("strict", []))
            level, reasons = _integrity_state(10, 9, 1, 0)
            self.assertEqual(level, "degraded")
            self.assertTrue(any("stale" in reason for reason in reasons))
            level, reasons = _integrity_state(10, 9, 0, 1)
            self.assertEqual(level, "degraded")
            self.assertTrue(any("failed" in reason for reason in reasons))
        ...
        def test_aborted_output_does_not_poison_cache(self):
            ...
            try:
                ...
                with mock.patch(__name__ + ".acquire_run_lock"), \
                     mock.patch(__name__ + "._recover_stale_backup", return_value=False), \
                     mock.patch(__name__ + ".resolve_commit_shas",
                                return_value=({canon: canon.replace("/main/", "/" + "a" * 40 + "/")},
                                              {canon: "resolved-api"})), \
        ...
        def test_report_failure_rolls_back_swapped_dist(self):
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            canon = "https://raw.githubusercontent.com/MajoSissi/animeko-source/main/dist/all.json"
            pinned = canon.replace("/main/", "/" + "a" * 40 + "/")
            try:
                os.chdir(d)
                write_valid_output_dir("seed", "reportfail")
                items = extract(load_json_file(os.path.join("seed", "all.json")))
                body = json.dumps({"exportedMediaSourceDataList": {"mediaSources": items}}).encode()
                shutil.rmtree("seed")
                with open("all_animeko_links.txt", "w", encoding="utf-8") as f:
                    f.write(canon + "\n")
                self.assertTrue(save_cache(canon, b"old-cache", {}, canon, "b" * 64, "c" * 40))
                cache_file = cache_path(canon)
                with open(cache_file, "rb") as handle:
                    old_cache = handle.read()
                real_write = _write_json_atomic

                def fail_report(path, obj):
                    if path.endswith(os.path.join("reports", "latest.json")):
                        raise OSError("injected-report-failure")
                    return real_write(path, obj)

                with mock.patch(__name__ + ".acquire_run_lock"), \
                     mock.patch(__name__ + ".resolve_commit_shas",
                                return_value=({canon: pinned}, {canon: "resolved-api"})), \
                     mock.patch(__name__ + ".fetch_group_worker",
                                return_value=(body, {"url": pinned, "final_url": pinned})), \
                     mock.patch(__name__ + ".load_cache", return_value=None), \
                     mock.patch(__name__ + "._write_json_atomic", side_effect=fail_report), \
                     mock.patch(__name__ + ".MIN_OUTPUT_COUNT", 1), \
                     mock.patch("builtins.print"):
                    with self.assertRaises(SystemExit):
                        main()
                self.assertFalse(os.path.exists("dist"))
                with open(cache_file, "rb") as handle:
                    self.assertEqual(handle.read(), old_cache)
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)

        def test_marker_failure_rolls_back_and_rewrites_report(self):
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            canon = "https://raw.githubusercontent.com/MajoSissi/animeko-source/main/dist/all.json"
            ...
            try:
                os.chdir(d)
                write_valid_output_dir("seed", "markerfail")
                items = extract(load_json_file(os.path.join("seed", "all.json")))
                body = json.dumps({"exportedMediaSourceDataList": {"mediaSources": items}}).encode()
                shutil.rmtree("seed")
                with open("all_animeko_links.txt", "w", encoding="utf-8") as f:
                ...
                def fail_marker(path):
                    if str(path).endswith(".txn-pending"):
                ...
                with mock.patch(__name__ + ".acquire_run_lock"), \
                     ...
                     mock.patch(__name__ + ".fetch_group_worker",
                                return_value=(body, {"url": pinned, "final_url": pinned})), \
                     mock.patch(__name__ + ".load_cache", return_value=None), \
                     mock.patch(__name__ + ".save_cache", return_value=True), \
                     mock.patch.object(os, "remove", side_effect=fail_marker), \
                     mock.patch(__name__ + ".MIN_OUTPUT_COUNT", 1), \
                     mock.patch("builtins.print"):
                    with self.assertRaises(SystemExit):
                        main()
                ...
                self.assertEqual(report["transaction_status"], "aborted")
                self.assertIsNone(report["actual_dist_sha256"])
                self.assertIn("标记清理失败", report["aborted_reason"])
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)

        def test_cache_failure_rolls_back_swapped_dist(self):
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            canon = "https://raw.githubusercontent.com/MajoSissi/animeko-source/main/dist/all.json"
            ...
            try:
                os.chdir(d)
                write_valid_output_dir("seed", "cachefail")
                items = extract(load_json_file(os.path.join("seed", "all.json")))
                body = json.dumps({"exportedMediaSourceDataList": {"mediaSources": items}}).encode()
                shutil.rmtree("seed")
                with open("all_animeko_links.txt", "w", encoding="utf-8") as f:
                ...
                with mock.patch(__name__ + ".acquire_run_lock"), \
                     ...
                     mock.patch(__name__ + ".fetch_group_worker",
                                return_value=(body, {"url": pinned, "final_url": pinned})), \
                     mock.patch(__name__ + ".load_cache", return_value=None), \
                     mock.patch(__name__ + ".save_cache", return_value=False), \
                     mock.patch(__name__ + ".MIN_OUTPUT_COUNT", 1), \
                     mock.patch("builtins.print"):
                    with self.assertRaises(SystemExit):
                        main()
                self.assertFalse(os.path.exists("dist"))
                report = load_json_file(os.path.join("reports", "latest.json"))
                self.assertIn("cache 事务失败", report["aborted_reason"])
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)
        ...
        def test_verified_history_snapshot_is_degraded_and_cannot_commit(self):
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            canon = "https://raw.githubusercontent.com/MajoSissi/animeko-source/main/dist/all.json"
            ...
            try:
                os.chdir(d)
                write_valid_output_dir("dist", "history")
                old_hash = _dir_hash("dist")
                old_outputs = {}
                counts = {}
                for fn in OUTPUT_FILES:
                    ms = extract(load_json_file(os.path.join("dist", fn)))
                    old_outputs[fn] = ms
                    counts[fn] = len(ms)
                with open(os.path.join("dist", "log"), "w", encoding="utf-8") as f:
                    f.write("old\n")
                with open(os.path.join("dist", "latest.json"), "w", encoding="utf-8") as f:
                    json.dump({"output_sha256": old_hash, "counts": counts,
                               "guard_anchor": _make_guard_anchor(old_outputs),
                               "source_snapshots": {canon: previous_sha},
                               "source_snapshots_created": int(time.time()),
                               "source_snapshot_methods": {canon: "resolved-api"},
                               "integrity_level": "strict"}, f)
                with open("all_animeko_links.txt", "w", encoding="utf-8") as f:
                    f.write(canon + "\n")
                body = json.dumps({"exportedMediaSourceDataList": {
                    "mediaSources": old_outputs["all.json"]}}).encode()
                fetcher = mock.MagicMock(return_value=(body, {"url": canon, "final_url": canon}))
                with mock.patch(__name__ + ".acquire_run_lock"), \
                     mock.patch(__name__ + ".resolve_commit_shas",
                                return_value=({canon: canon},
                                              {canon: "snapshot-unresolved:api-error:ref-service-timeout"})), \
                     mock.patch(__name__ + ".fetch_group_worker", fetcher), \
                     mock.patch(__name__ + ".load_cache", return_value=None), \
                     mock.patch(__name__ + ".MIN_OUTPUT_COUNT", 1), \
                     mock.patch("builtins.print"):
                    main()
                ...
                report = load_json_file(os.path.join("reports", "latest.json"))
                self.assertEqual(report["integrity_level"], "degraded")
                self.assertFalse(report["changed"])
                self.assertEqual(_dir_hash("dist"), old_hash)
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)

        def test_unresolved_github_snapshot_aborts_before_fetch(self):
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            canon = "https://raw.githubusercontent.com/o/r/main/x.json"
            ...
            try:
                ...
                fetcher.assert_not_called()
                report = load_json_file(os.path.join("reports", "latest.json"))
                self.assertIn("snapshot 解析失败", report["aborted_reason"])
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)

        def test_early_abort_report_failure_falls_back_to_stderr(self):
            import contextlib
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            ...
            try:
                os.chdir(d)
                with mock.patch(__name__ + ".acquire_run_lock"), \
                     mock.patch(__name__ + "._write_json_atomic",
                                side_effect=OSError("read-only-report")), \
                     contextlib.redirect_stderr(stderr), contextlib.redirect_stdout(stdout):
                    with self.assertRaises(SystemExit):
                        main()
                self.assertIn("report_write_error", stderr.getvalue())
                self.assertIn("上游链接读取失败", stderr.getvalue())
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)
        ...
        def test_total_input_budget_is_enforced_before_each_read(self):
            class Raw:
            ...
            budget = _InputBudget(6)
            first = Response(b"abcd")
            self.assertEqual(b"".join(_iter_response_chunks(
                first, time.monotonic() + 1, budget)), b"abcd")
            self.assertEqual(budget.totals(), (0, 4))
            second = Response(b"efgh")
            with self.assertRaises(_InputBudgetExceeded):
                b"".join(_iter_response_chunks(second, time.monotonic() + 1, budget))
            self.assertEqual(budget.totals(), (0, 6))
            self.assertEqual(second.raw.reads, 1)
            third = Response(b"z")
            with self.assertRaises(_InputBudgetExceeded):
                next(_iter_response_chunks(third, time.monotonic() + 1, budget))
            self.assertEqual(third.raw.reads, 0)

        def test_304_cache_bytes_are_not_charged_as_network(self):
            budget = _InputBudget(10)
            self.assertTrue(budget.reserve_cache(6))
            self.assertEqual(budget.totals(), (6, 0))
            self.assertEqual(budget.claim_network(4), 4)
            self.assertEqual(budget.totals(), (6, 4))
        ...
        def test_rollback_dist_swap_restores_backup(self):
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            try:
                os.chdir(d)
                os.makedirs("dist")
                with open(os.path.join("dist", "current"), "w", encoding="utf-8") as f:
                    f.write("new")
                os.makedirs("dist.bak-1")
                with open(os.path.join("dist.bak-1", "current"), "w", encoding="utf-8") as f:
                    f.write("old")
                self.assertTrue(_rollback_dist_swap("dist.bak-1", True))
                with open(os.path.join("dist", "current"), encoding="utf-8") as f:
                    self.assertEqual(f.read(), "old")
                self.assertFalse(os.path.lexists("dist.bak-1"))
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)

        def test_rollback_dist_swap_reports_restore_failure(self):
            import tempfile
            d = tempfile.mkdtemp()
            cwd = os.getcwd()
            try:
                ...
                def fail_restore(src, dst):
                    if src == "dist.bak-1" and dst == "dist":
                        raise OSError("blocked")
                    return real_rename(src, dst)

                with mock.patch("os.rename", side_effect=fail_restore):
                    self.assertFalse(_rollback_dist_swap("dist.bak-1", True))
                self.assertFalse(os.path.lexists("dist"))
                self.assertTrue(os.path.isdir("dist.bak-1"))
            finally:
                os.chdir(cwd)
                shutil.rmtree(d, ignore_errors=True)
...
if __name__ == "__main__":
    if len(sys.argv) == 2 and sys.argv[1] == "--test":
        ...
        try:
            ...
            print(f"❌ 产物校验异常: {type(exc).__name__}: {exc}")
            sys.exit(1)
        if problems:
            print("❌ 校验失败：")
