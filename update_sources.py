...
_DNS_RESOLVER_CODE = (
    ...
    "x=socket.getaddrinfo(sys.argv[1],None,type=socket.SOCK_STREAM);"
    "print(json.dumps([i[4][0] for i in x]))"
)
...
def touch_cache(canon: str, accepted_output_hash: str | None = None):
    _rewrite_cache_metadata(canon, accepted_output_hash, True)


def bind_cache(canon: str, accepted_output_hash: str):
    _rewrite_cache_metadata(canon, accepted_output_hash, False)
...
def _metadata_ok(d: str, output_hash: str | None, counts: dict) -> bool:
    try:
        ...
        return (
            ...
            and not _guard_anchor_expired(guard_anchor)
        )
    except Exception:
        return False
...
def _guard_anchor_expired(anchor: dict) -> bool:
    return time.time() - anchor["created"] > GUARD_ANCHOR_MAX_AGE_DAYS * 86400
...
def main():
    ...
    def fetch_batch(batch: list[str]):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            futs = {}
            for canon in batch:
                cache_meta = load_cache(canon, old_dist_hash) if old_dist_hash is not None else None
    ...
    if old_outputs.get("all.json"):
        ...
        if guard_anchor is None or _guard_anchor_expired(guard_anchor):
    ...
    for action, canon, data, meta, validator_url in pending_cache_updates:
        if action == "save":
            save_cache(canon, data, meta, validator_url, new_hash)
        elif action == "touch":
            touch_cache(canon, new_hash)
        else:
            bind_cache(canon, new_hash)
    os.makedirs(REPORT_DIR, exist_ok=True)
    err_cats = {}
    for c, m in ok.items():
...
def run_selftests():
    ...
    class T(unittest.TestCase):
        ...
        def test_cache_bound_to_accepted_output(self):
            ...
            second_hash = "b" * 64
            try:
                save_cache(canon, b'{"a":1}', {}, canon, first_hash)
                self.assertIsNotNone(load_cache(canon, first_hash))
                self.assertIsNone(load_cache(canon, second_hash))
                before_ts = load_cache(canon)["ts"]
                bind_cache(canon, second_hash)
                rebound = load_cache(canon, second_hash)
        ...
        def test_cumulative_content_replacement_guard(self):
            ...
            expired = _make_guard_anchor({"all.json": original},
                                         int(time.time()) - (GUARD_ANCHOR_MAX_AGE_DAYS + 1) * 86400)
            self.assertTrue(_guard_anchor_expired(expired))
        ...
        def test_dns_resolution_respects_deadline(self):
            ...
            with mock.patch.object(subprocess, "run", side_effect=timeout_run):
        ...
        def test_dns_outstanding_limit_respects_deadline(self):
            ...
            with mock.patch(__name__ + "._dns_slots", slots), \
                 mock.patch.object(subprocess, "run") as resolver:
        ...
        def test_dns_workers_recover_after_full_saturation(self):
            slots = threading.BoundedSemaphore(DNS_MAX_OUTSTANDING)
            ...
            with mock.patch(__name__ + "._dns_slots", slots):
                ...
                completed = subprocess.CompletedProcess([], 0, '["93.184.216.34"]\n', "")
