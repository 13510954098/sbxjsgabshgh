...
def touch_cache(canon: str, accepted_output_hash: str | None = None,
                snapshot_sha: str | None = None):
    _rewrite_cache_metadata(canon, accepted_output_hash, True, snapshot_sha)


def bind_cache(canon: str, accepted_output_hash: str, snapshot_sha: str | None = None):
    _rewrite_cache_metadata(canon, accepted_output_hash, False, snapshot_sha)
...
def main():
    ...
    for action, canon, data, meta, validator_url in pending_cache_updates:
        ...
        if action == "save":
            save_cache(canon, data, meta, validator_url, new_hash, snapshot_sha)
        elif action == "touch":
            touch_cache(canon, new_hash, snapshot_sha)
        else:
            bind_cache(canon, new_hash, snapshot_sha)
    os.makedirs(REPORT_DIR, exist_ok=True)
    err_cats = {}
    for c, m in ok.items():
