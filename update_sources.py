...
GROUP_DEADLINE = 90
STRICT_SNAPSHOT_METHODS = frozenset(("resolved-api", "resolved-ref-service", "already-sha"))
IMMUTABLE_SNAPSHOT_METHODS = STRICT_SNAPSHOT_METHODS | frozenset(("resolved-history",))
...
def _resolve_ref_via_http(owner: str, repo: str, ref: str) -> tuple[str | None, str]:
    ...
    for visited in meta.get("redirects") or []:
        if (urlsplit(visited).hostname or "").lower().rstrip(".") != "github.com":
...
def _snapshot_methods_valid(snapshots, methods, integrity_level: str) -> bool:
    if not isinstance(snapshots, dict) or not isinstance(methods, dict):
        return False
    allowed = (STRICT_SNAPSHOT_METHODS if integrity_level == "strict"
               else IMMUTABLE_SNAPSHOT_METHODS if integrity_level == "degraded"
               else frozenset())
    return all(isinstance(canon, str) and isinstance(sha, str)
               and re.fullmatch(r"[0-9a-f]{40}", sha)
               and methods.get(canon) in allowed
...
def run_selftests():
    ...
    class T(unittest.TestCase):
        ...
        def test_snapshot_method_trust_levels(self):
            ...
            self.assertTrue(_snapshot_methods_valid(snapshots, strict, "degraded"))
        ...
        def test_ref_service_fallback_uses_pinned_fetch_stack(self):
            ...
            meta = {"redirects": ["https://github.com/Owner/Repo.git/info/refs"]}
        ...
        def test_github_api_resolution_cap(self, session_cls):
            ...
            with mock.patch(__name__ + ".MAX_GITHUB_API_REFS", 1), \
                 mock.patch(__name__ + "._resolve_ref_via_http",
                            return_value=("d" * 40, "resolved-ref-service")) as git_resolver:
                _, status = resolve_commit_shas(canons)
            self.assertEqual(session_cls.call_count, 1)
            self.assertEqual(git_resolver.call_count, 3)
                ...
                with mock.patch(__name__ + ".acquire_run_lock"), \
                     mock.patch(__name__ + ".resolve_commit_shas",
                                return_value=({canon: canon},
                                              {canon: "snapshot-unresolved:api-error:git-timeout"})), \
                     mock.patch(__name__ + ".fetch_group_worker", fetcher), \
                     mock.patch(__name__ + ".load_cache", return_value=None), \
                     mock.patch(__name__ + ".MIN_OUTPUT_COUNT", 1), \
                     mock.patch("builtins.print"):
                    main()
                urls = fetcher.call_args.args[1]
                self.assertTrue(all(previous_sha in url for url in urls))
                report = load_json_file(os.path.join("reports", "latest.json"))
