...
_DNS_RESOLVER_CODE = (
    ...
    "  x=socket.getaddrinfo(host,None,type=socket.SOCK_STREAM)\n"
    "  print(json.dumps({'addresses':[i[4][0] for i in x]}),flush=True)\n"
...
class _DNSResolverWorker:
    ...
    def start(self):
        ...
        proc = subprocess.Popen(
            [sys.executable, "-I", "-u", "-c", _DNS_RESOLVER_CODE],
...
def main():
    ...
    if cache_update_failures:
        if swap_completed:
            _rollback_dist_swap(swap_backup, swap_old_existed)
            swap_completed = False
        _abort(f"cache 事务失败：{cache_update_failures[:5]}")
    os.makedirs(REPORT_DIR, exist_ok=True)
    err_cats = {}
    for c, m in ok.items():
    ...
    try:
        ...
        if swap_completed:
            _rollback_dist_swap(swap_backup, swap_old_existed)
            swap_completed = False
        print(f"报告事务失败: {type(exc).__name__}: {exc}", file=sys.stderr)
        raise SystemExit(1)
    if swap_completed:
        try:
            ...
            _rollback_dist_swap(swap_backup, swap_old_existed)
            swap_completed = False
            _abort(f"事务提交标记清理失败: {type(exc).__name__}: {exc}")
...
def run_selftests():
    ...
    class T(unittest.TestCase):
        ...
        def test_dns_process_worker_restarts_after_timeout(self):
            ...
            worker = None
            with mock.patch(__name__ + "._DNS_RESOLVER_CODE", blocking_code):
                worker = _DNSResolverWorker()
                old_proc = worker.proc
                pool = queue.LifoQueue(maxsize=1)
                pool.put(worker)
                with mock.patch(__name__ + "._get_dns_pool", return_value=pool):
                    infos, error = _bounded_getaddrinfo("blocked.example", time.monotonic() + 0.05)
                self.assertIsNone(infos)
                self.assertEqual(error, "dns-timeout")
                self.assertIsNot(worker.proc, old_proc)
                self.assertIsNotNone(old_proc.poll())
            try:
                with mock.patch(__name__ + "._DNS_RESOLVER_CODE", responsive_code):
                    worker.start()
                    with mock.patch(__name__ + "._get_dns_pool", return_value=pool):
