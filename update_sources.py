...
def _resolver_command() -> list[str]:
    ...
    return [sys.executable, "-I", "-u", script, "--dns-resolver-worker"]
...
def fetch_url(url: str, cache_meta: dict | None, deadline: float,
    ...
    try:
        while redirect_count <= MAX_REDIRECTS:
            ...
            if expected_snapshot_sha is not None and not _url_matches_snapshot(current, expected_snapshot_sha):
            ...
            if location is not None:
                ...
                if urlsplit(current).scheme == "https" and urlsplit(next_url).scheme != "https":
                    ...
                    return None, meta
                if expected_snapshot_sha is not None and not _url_matches_snapshot(next_url, expected_snapshot_sha):
...
def fetch_group_worker(canon: str, urls: list[str], cache_meta,
    ...
    last_meta = None
    for u in urls:
        data, meta = fetch_url(u, cache_meta, deadline, expected_snapshot_sha)
        last_meta = meta
        if data is not None:
            return data, meta
...
def _url_matches_snapshot(url: str, snapshot_sha: str) -> bool:
    if not re.fullmatch(r"[0-9a-f]{40}", snapshot_sha):
        return False
    try:
        for parser in (parse_github_raw, parse_github_com, parse_jsdelivr):
            parsed = parser(url)
            if parsed:
                return parsed.ref.lower() == snapshot_sha
        normalized = normalize(url)
        parsed = parse_github_raw(normalized)
        return bool(parsed and parsed.ref.lower() == snapshot_sha)
    except Exception:
        return False
...
def _github_snapshot_urls(canon: str, pinned_url: str) -> list[str]:
    ...
    if not original or not pinned or not re.fullmatch(r"[0-9a-f]{40}", pinned.ref):
        raise ValueError(f"GitHub snapshot 无效: {canon}")
    owner_q = quote(unquote(original.owner), safe="")
    repo_q = quote(unquote(original.repo), safe="")
    path_q = quote(unquote(original.path), safe="/")
    jsdelivr = f"https://cdn.jsdelivr.net/gh/{owner_q}/{repo_q}@{pinned.ref}/{path_q}"
    return [pinned_url, jsdelivr]
...
def _recover_stale_backup():
    ...
    if valid_backups and not dist_valid:
        ...
        dist_valid = True
    elif pending_swap and os.path.lexists("dist"):
        _remove_path("dist")
    if dist_valid:
        for bak in backups:
...
def main():
    ...
    def fetch_batch(batch: list[str]):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            futs = {}
            for canon in batch:
                expected_snapshot = snapshot_shas.get(canon)
                cache_meta = (load_cache(canon, old_dist_hash, expected_snapshot)
                ...
                if expected_snapshot is not None:
                    ...
                    for u in groups[canon]:
                        if u not in urls:
                            urls.append(u)
                futs[ex.submit(fetch_group_worker, canon, urls, cache_meta,
                               expected_snapshot)] = canon
    ...
    if integrity_level != "strict":
        ...
        changed_any = not (old_hash and old_hash == new_hash
                           and _metadata_ok("dist", old_hash, new_counts))
    ...
    if cache_update_failures:
        ...
        if swap_completed:
            rollback_failed = not _rollback_dist_swap(swap_backup, swap_old_existed)
            swap_completed = False
    ...
    try:
        ...
        if swap_completed:
            rollback_status = ("complete" if _rollback_dist_swap(
    ...
    if swap_completed:
        try:
            ...
            rollback_ok = _rollback_dist_swap(swap_backup, swap_old_existed)
            swap_completed = False
...
def run_selftests():
    ...
    class T(unittest.TestCase):
        ...
        def test_snapshot_redirect_must_preserve_commit(self, mget):
            ...
            data, meta = fetch_url(start, None, time.monotonic() + 30, sha)
            self.assertIsNone(data)
        ...
        def test_total_input_budget_aborts_before_parsing(self):
            ...
            try:
                ...
                with open("all_animeko_links.txt", "w", encoding="utf-8") as f:
                    f.write(a + "\n" + b + "\n")
                mapping = {
                    a: a.replace("/main/", "/" + "a" * 40 + "/"),
                    b: b.replace("/main/", "/" + "b" * 40 + "/"),
                }
                statuses = {a: "resolved-api", b: "resolved-api"}
                with mock.patch(__name__ + ".acquire_run_lock"), \
                     mock.patch(__name__ + "._recover_stale_backup", return_value=False), \
...
if __name__ == "__main__":
    if len(sys.argv) == 2 and sys.argv[1] == "--dns-resolver-worker":
