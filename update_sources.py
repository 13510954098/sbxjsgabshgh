...
# -*- coding: utf-8 -*-
"""
Animeko 聚合源自动更新脚本 v10（修复 v9 复审发现的问题）
============================================
...
v10（修复 v9 复审）：
  1  安全护栏退出可观测性：main() 所有"拒绝覆盖产物"的退出（core/stale/有效率/条目骤减/
     删除比例/产物校验）统一改走 _abort()——先落盘 reports/latest.json（含 aborted_reason
     + 抓取/解析统计）再 sys.exit。此前这些退出都发生在末尾报告写入之前，最该排查的场景反而无报告。
  2  SSRF 字面层收紧：is_literal_private_host 识别混合十六进制八位（0x7f.0.0.1 = 127.0.0.1）
     等数字地址——旧启发式因含 'x'/非纯 hex 而漏判放行；现"仅由 IP-ish 字符组成且含数字"即判
     数字形式，ipaddress 解析失败一律拒绝。（fetch 路径本有 DNS 兜底，此为字面层加固）
  3  浮点 tier 收敛：safe_tier/safe_channel_tier 对整数值 float（JSON "tier":2.0）收敛为 int，
     不再因 isinstance(2.0,int)==False 而静默丢弃整个源。
  4  jsdelivr/raw 空 path：@main（@ref 后无文件名）、raw 仅 owner/repo/ref 段等结构非法链接，
     在 parse_jsdelivr/parse_github_raw 返回 None 后，normalize 兜底分支对文件托管域名
     （cdn.jsdelivr.net / raw.githubusercontent.com）也 raise ValueError，彻底拒绝（不再生成幽灵 canon）。
  5  channelTiers 校验报告全部非法项（不再遇到首个即 break）。
  6  解析阶段复用 fetch_batch 已加载的 cache_metas，避免对同一 canon 重复 load_cache（磁盘读+解压）。
  7  删除死代码 get_session()/_tls（v9 起抓取全量走 get_pinned_session，未被调用）。
  8  补回归测试（混合十六进制环回、float tier、jsdelivr 空 path、channelTiers 全报）。
...
# [v10] 数字型 IP 地址可能用到的字符集（含混合十六进制八位，如 0x7f.0.0.1）
...
def safe_tier(t):
    ...
    if isinstance(t, float):
        # [v10] JSON 可能解析出 float（2.0）；整数 float 收敛，否则拒绝
...
def safe_channel_tier(v):
    ...
    if isinstance(v, float):
        # [v10] 整数值 float 收敛（与 safe_tier 一致）
...
def is_literal_private_host(host: str) -> bool:
    ...
    # [v10] 收紧数字地址判定：只要"仅由 IP-ish 字符组成且含数字"即视为数字形式。
    #       修复 0x7f.0.0.1（混合十六进制八位 = 127.0.0.1）漏判——旧启发式因含 'x'/'.'
    #       既不判纯 hex 也不判纯数字而放行；现一律按数字形式，ipaddress 解析失败即拒绝。
    looks_numeric = (
...
def parse_jsdelivr(url: str):
    ...
    # [v10] @ref 后必须跟文件路径；@main（无文件名）返回 None，
    #       不再生成空 path 的幽灵 canon（raw.githubusercontent.com/o/r/main/）
    if "/" not in rest:
        return None
    ...
    while True:
        ...
        if host in ACCEL_HOSTS:
            ...
            if up.path.startswith("/https://github.com/"):
                ...
                continue
        break
    # [v10] 文件托管域名但解析器未能解析出合法 GitHubRef（path 缺失/结构非法）→ 拒绝，
    #       不再静默收编为会 404 的幽灵 canon（如 jsdelivr @main 无文件名、raw 仅 owner/repo/ref）。
    #       正常的 raw/jsdelivr 链接在上面 for 循环里已被解析 return，到此处必为结构非法。
...
def validate_item(m) -> tuple[bool, list[str]]:
    ...
    if ct is not None and not isinstance(ct, dict):
        ...
        # [v10] 报告全部非法项（不再遇到第一个就 break），诊断更完整
...
# [高1] 每个 host+IP 一个固定 IP 的专用 session（thread-local，避免多线程共享）
...
def main():
    ...
    def _save_abort_report(reason):
        # [v10] 安全护栏退出前先落盘诊断报告（可观测性）：失败原因 + 当前抓取/解析统计。
        #       此前 main() 的所有 sys.exit 都发生在末尾"结构化报告"写入之前，
        #       导致最该排查的"拒绝更新"场景反而没有报告产物。
    ...
    cache_metas: dict[str, dict] = {}  # [v10] 复用缓存元数据，避免解析阶段对同一 canon 重复 load_cache
    ...
    for canon in canons_sorted:
        if canon in ok:
            ...
            # 网络失败 → stale-if-error
            cache = cache_metas.get(canon)  # [v10] 复用 fetch_batch 已加载的缓存（行为等价，省一次磁盘读+解压）
...
def run_selftests():
    ...
    class T(unittest.TestCase):
        ...
        # ---------------- v10 回归用例 ----------------
        def test_mixed_hex_octet_loopback_rejected(self):  # [v10] 0x7f.0.0.1 等混合十六进制环回
            for host in ("0x7f.0.0.1", "0x7f.1", "0x7f.0.0.001"):
                self.assertTrue(is_literal_private_host(host), host)
            # 字面层也拦住（fetch 路径另有 DNS 兜底，此处验证字面层不再漏判）
        ...
        def test_float_tier_coerced(self):  # [v10] JSON tier=2.0 收敛为 int，不再被丢弃
            self.assertEqual(safe_tier(2.0), 2)
            self.assertEqual(safe_tier(0.0), 0)
            self.assertIsNone(safe_tier(2.5))      # 非整数 float 仍拒绝
            self.assertIsNone(safe_tier(-1.0))
            self.assertEqual(safe_channel_tier(5.0), 5)
            self.assertIsNone(safe_channel_tier(2.5))

        def test_jsdelivr_no_path_rejected(self):  # [v10] @main 后无文件名 → 彻底拒绝（parse + normalize 两层）
            # parse 层返回 None
            ...
            # [v10] normalize 层也彻底拒绝，不再静默收编为会 404 的幽灵 canon
            for bad in ("https://cdn.jsdelivr.net/gh/o/r@main",
                        "https://raw.githubusercontent.com/o/r/main"):
                with self.assertRaises(ValueError):
                    normalize(bad)

        def test_channel_tiers_reports_all_invalid(self):  # [v10] channelTiers 报全部非法项
            item = {"factoryId": "web-selector", "version": 2,
                    "arguments": {"name": "X", "searchConfig": {"searchUrl": "https://x.com"},
