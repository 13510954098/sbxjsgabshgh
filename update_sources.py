...
def main():
    ...
    # ---------------- 写入（单文件原子替换） ----------------
    # 说明：os.replace 对单个文件原子；六文件循环替换非整体事务（进程中途崩溃
    # 可能留混合状态）。但 Actions 的 git commit 本身是原子的——订阅者只会看到
    # commit 后的完整集合；本地运行若担心，可自行用 tmp 目录 + 校验（当前即如此）。
    os.makedirs("dist", exist_ok=True)
    tmpdir = os.path.join("dist", f".tmp-{os.getpid()}")
    os.makedirs(tmpdir, exist_ok=True)
    changed_any = False
    try:
        # 阶段1：全部写入 tmp（此时 dist/ 还是旧版本，无任何改动）
