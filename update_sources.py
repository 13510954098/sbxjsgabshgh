...
配套文件：.github/workflows/update.yml、.gitignore、requirements.txt、LICENSE、README.md
...
def parse_github_com(url: str):
    ...
    if parts[2] == "raw" and len(parts) >= 4:
        ref = "/".join(parts[3:-1])
        return GitHubRef(parts[0], parts[1], fold_ref(ref), parts[-1])
    if parts[2] == "blob" and len(parts) >= 4:
        return GitHubRef(parts[0], parts[1], fold_ref(parts[3]), "/".join(parts[4:]))
    return None
...
def fetch_group_worker(canon: str, urls: list[str], cache_meta):
    ...
    for u in urls:
        ...
        if "http-404" in err or "redirect-" in err:
            continue
        if "deadline" in err:
            break
...
def main():
    ...
    for canon in canons_sorted:
        if canon in ok:
            ...
            seg, chosen = choose_fresh_or_stale(
                fresh_data=data if not meta.get("cached") else None,
                stale_data=(load_cache(canon) or {}).get("data") if not meta.get("cached") else None,
            ...
            if chosen == "fresh":
                ...
                failed[canon] = {"error": "fresh-invalid-using-stale-cache"}
            ...
            if cache and cache.get("data"):
                ...
                if chosen == "stale":
                    candidates.extend(seg)
                    stale_valid_canons.add(canon)
                    failed[canon] = {"error": f"using-stale-cache (orig: {failed.get(canon, {}).get('error')})"}
                    print(f"  缓存兜底: {canon}")
                else:
    ...
    err_cats = {}
    for c, m in ok.items():
        e = m[1].get("error") or ("ok" if not m[1].get("cached") else "stale-cache")
    ...
    report = {
        ...
        "fetched": {c: {"size": m[1].get("size"), "sha256": m[1].get("sha256"),
                        "etag": m[1].get("etag"), "status": m[1].get("status"),
                        "error": m[1].get("error"), "cached": m[1].get("cached"),
...
def run_selftests():
    ...
    class T(unittest.TestCase):
        ...
        def test_default_links_embedded(self):
            # [拆分] 链接已外置：本文件目录下应有 all_animeko_links.txt（162 条）
            self.assertTrue(os.path.exists("all_animeko_links.txt"))
            self.assertEqual(len(clean_links(open("all_animeko_links.txt", encoding="utf-8"))), 162)
