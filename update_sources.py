...
GITHUB_GIT_WORKERS = 8
GITHUB_GIT_TIMEOUT = 20
MAX_REDIRECTS = 5
...
_dns_result_cache = {}
_dns_result_cache_lock = threading.Lock()
_DNS_RESOLVER_CODE = (
...
def _bounded_getaddrinfo(host: str, deadline: float | None = None):
    ...
    try:
        ...
        with _dns_result_cache_lock:
            _dns_result_cache[host] = (time.monotonic(), tuple(infos))
        return infos, None
    except TimeoutError:
        restart = True
        return None, "dns-timeout"
...
def _resolve_git_ref(owner: str, repo: str, ref: str) -> tuple[str | None, str]:
    git_exe = shutil.which("git")
    decoded_ref = unquote(ref)
    if not git_exe or not _git_ref_safe(decoded_ref):
        return None, "git-unavailable-or-invalid-ref"
    owner_q = quote(unquote(owner), safe="")
    repo_q = quote(unquote(repo), safe="")
    repo_url = f"https://github.com/{owner_q}/{repo_q}.git"
    if decoded_ref.startswith(("refs/heads/", "refs/tags/")):
        patterns = [decoded_ref]
        if decoded_ref.startswith("refs/tags/"):
            patterns.append(decoded_ref + "^{}")
        branch_ref = decoded_ref if decoded_ref.startswith("refs/heads/") else None
        tag_ref = decoded_ref if decoded_ref.startswith("refs/tags/") else None
    else:
        branch_ref = f"refs/heads/{decoded_ref}"
        tag_ref = f"refs/tags/{decoded_ref}"
        patterns = [branch_ref, tag_ref, tag_ref + "^{}"]
    command = [git_exe, "-c", "credential.helper="]
    if os.environ.get("ALLOW_ENV_PROXY") != "1":
        command.extend(("-c", "http.proxy=", "-c", "https.proxy="))
    command.extend(("ls-remote", "--exit-code", repo_url, *patterns))
    env = os.environ.copy()
    ...
    if os.environ.get("ALLOW_ENV_PROXY") != "1":
        for key in tuple(env):
            if key.lower() in ("http_proxy", "https_proxy", "all_proxy"):
                env.pop(key, None)
    try:
        result = subprocess.run(command, capture_output=True, text=True, timeout=GITHUB_GIT_TIMEOUT,
                                check=False, env=env)
    ...
    if result.returncode != 0 or len(result.stdout) > 65536:
        return None, f"git-exit-{result.returncode}"
    found = {}
    ...
    if branch_ref and branch_ref in found:
        return found[branch_ref], "resolved-git"
    if tag_ref and tag_ref + "^{}" in found:
        return found[tag_ref + "^{}"], "resolved-git"
    if tag_ref and tag_ref in found:
        return found[tag_ref], "resolved-git"
...
def resolve_commit_shas(canons: list[str]) -> tuple[dict[str, str], dict[str, str]]:
    ...
    needs_git = [(key, ref_info[key]) for key, (_, st) in repo_refs.items()
                 if st not in ("resolved", "already-sha")]
    if needs_git:
        with ThreadPoolExecutor(max_workers=min(GITHUB_GIT_WORKERS, len(needs_git))) as ex:
            futures = {ex.submit(_resolve_git_ref, *info): key for key, info in needs_git}
            for future in as_completed(futures):
                key = futures[future]
                previous_ref, previous_status = repo_refs[key]
                try:
                    git_sha, git_status = future.result()
                except Exception:
                    git_sha, git_status = None, "git-error"
                if git_sha is not None:
                    repo_refs[key] = (git_sha, git_status)
                else:
                    repo_refs[key] = (previous_ref,
                                      f"snapshot-unresolved:{previous_status}:{git_status}")
...
def _anchor_guard_problems(anchor: dict, new_outputs: dict) -> list[str]:
    ...
    if removed / len(old_fingerprints) > MAX_CUMULATIVE_DELETE_RATIO:
        return [f"[P0-2] 30 天基线累计内容替换 {removed}/{len(old_fingerprints)} 超 "
...
def main():
    ...
    git_fallbacks = [c for c, st in sha_status.items() if st == "resolved-git"]
    if git_fallbacks:
        print(f"⚠️ GitHub API 不可用，已由 git 解析不可变 SHA（{len(git_fallbacks)} 个源）")
...
def run_selftests():
    ...
    class T(unittest.TestCase):
        ...
        def test_github_api_ignores_env_proxy_by_default(self, session_cls):
            ...
            with mock.patch(__name__ + "._resolve_git_ref",
                            return_value=("c" * 40, "resolved-git")):
                _, status = resolve_commit_shas(["https://raw.githubusercontent.com/o/r/main/x.json"])
            self.assertFalse(api.trust_env)
            self.assertEqual(next(iter(status.values())), "resolved-git")
        ...
        def test_github_api_invalid_sha_rejected(self, session_cls):
            ...
            with mock.patch(__name__ + "._resolve_git_ref",
                            return_value=(None, "git-ref-not-found")):
        ...
        def test_git_ref_fallback_returns_full_sha_without_proxy(self):
            result = subprocess.CompletedProcess(
                [], 0, "e" * 40 + "\trefs/heads/main\n", "")
            with mock.patch.object(shutil, "which", return_value="/usr/bin/git"), \
                 mock.patch.object(subprocess, "run", return_value=result) as runner, \
                 mock.patch.dict(os.environ,
                                 {"HTTPS_PROXY": "http://proxy.invalid", "ALLOW_ENV_PROXY": "0"},
                                 clear=False):
                sha, status = _resolve_git_ref("Owner", "Repo", "main")
            self.assertEqual(sha, "e" * 40)
            self.assertEqual(status, "resolved-git")
            command = runner.call_args.args[0]
            self.assertIn("http.proxy=", command)
            self.assertNotIn("HTTPS_PROXY", runner.call_args.kwargs["env"])
        ...
        def test_github_api_resolution_cap(self, session_cls):
            ...
            with mock.patch(__name__ + ".MAX_GITHUB_API_REFS", 1), \
                 mock.patch(__name__ + "._resolve_git_ref",
                            return_value=("d" * 40, "resolved-git")) as git_resolver:
                _, status = resolve_commit_shas(canons)
            self.assertEqual(session_cls.call_count, 1)
            self.assertEqual(git_resolver.call_count, 3)
            self.assertTrue(all(v == "resolved-git" for v in status.values()))
        ...
        def test_dns_result_cache_avoids_repeated_ipc(self):
            host = "cache-test.example"
            worker = mock.MagicMock()
            worker.query.return_value = (["93.184.216.34"], None)
            pool = queue.LifoQueue(maxsize=1)
            pool.put(worker)
            with _dns_result_cache_lock:
                _dns_result_cache.pop(host, None)
            with mock.patch(__name__ + "._get_dns_pool", return_value=pool):
                first, first_error = _bounded_getaddrinfo(host, time.monotonic() + 1)
                second, second_error = _bounded_getaddrinfo(host, time.monotonic() + 1)
            self.assertIsNone(first_error)
            self.assertIsNone(second_error)
            self.assertEqual(first, second)
            self.assertEqual(worker.query.call_count, 1)
        ...
        def test_report_failure_rolls_back_swapped_dist(self):
            ...
            try:
                ...
                with mock.patch(__name__ + ".acquire_run_lock"), \
                     mock.patch(__name__ + ".resolve_commit_shas",
                                return_value=({canon: pinned}, {canon: "resolved"})), \
                     mock.patch(__name__ + ".fetch_group_worker",
